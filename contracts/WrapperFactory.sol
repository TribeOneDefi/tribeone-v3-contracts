...
contract WrapperFactory is Owned, MixinResolver, IWrapperFactory {
    ...
    bytes32 internal constant CONTRACT_SYNTH_SUSD = "SynthuUSD";
    ...
    function feesEscrowed() public view returns (uint) {
        return synthuUSD().balanceOf(address(this));
    }
    ...
    function distributeFees() external {
        ...
        if (amountSUSD > 0) {
            // Transfer uUSD to the fee pool
            bool success = synthuUSD().transfer(feePool().FEE_ADDRESS(), amountSUSD);
