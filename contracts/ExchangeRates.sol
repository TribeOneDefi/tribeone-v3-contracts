...
contract ExchangeRates is Owned, MixinSystemSettings, IExchangeRates {
        ...
        )
    {
        ...
        require(currencyKey == uUSD || aggregatorAddress != address(0), "No aggregator for asset");
        ...
        returns (uint[] memory rates, bool anyRateInvalid)
    {
        ...
        for (uint i = 0; i < currencyKeys.length; i++) {
            ...
            if (!anyRateInvalid && currencyKeys[i] != uUSD) {
                anyRateInvalid =
                    flagList[i] ||
    ...
    function _getRateAndTimestampAtRound(bytes32 currencyKey, uint roundId) internal view returns (uint rate, uint time) {
        // short circuit uUSD
        if (currencyKey == uUSD) {
            // uUSD has no rounds, and 0 time is preferrable for "volatility" heuristics
    ...
    function _rateIsStale(bytes32 currencyKey, uint _rateStalePeriod) internal view returns (bool) {
        // uUSD is a special case and is never stale (check before an SLOAD of getRateAndUpdatedTime)
        if (currencyKey == uUSD) {
            return false;
        }
    ...
    function _rateIsFlagged(bytes32 currencyKey, FlagsInterface flags) internal view returns (bool) {
        // uUSD is a special case and is never invalid
        if (currencyKey == uUSD) {
            return false;
        }
        address aggregator = address(aggregators[currencyKey]);
