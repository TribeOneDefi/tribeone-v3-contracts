...
contract Wrapper is Owned, Pausable, MixinResolver, MixinSystemSettings, IWrapper {
    ...
    bytes32 private constant CONTRACT_SYNTH_SUSD = "SynthuUSD";
    ...
    function totalIssuedSynths() public view returns (uint) {
        ...
        return exchangeRates().effectiveValue(currencyKey, targetSynthIssued, uUSD);
    }
    ...
    function _mint(uint amount) internal {
        ...
        uint excessAmountUsd = exchangeRates().effectiveValue(currencyKey, excessAmount, uUSD);
        ...
        if (excessAmountUsd > 0) {
            synthuUSD().issue(address(wrapperFactory()), excessAmountUsd);
        }
