...
interface IIssuerInternalDebtCache {
    ...
    function updateCacheduUSDDebt(int amount) external;
}
...
contract Issuer is Owned, MixinSystemSettings, IIssuer {
    ...
    {
        (alreadyIssued, totalSystemDebt, anyRateIsInvalid) = _debtBalanceOfAndTotalDebt(_debtShareBalanceOf(_issuer), uUSD);
    ...
    function _maxIssuableSynths(address _issuer) internal view returns (uint, bool) {
        // What is the value of their HAKA balance in uUSD
    ...
    /// @return debtToRemove the amount of debt (uUSD) to burn in order to fix the account's c-ratio
    /// @return escrowToLiquidate the amount of escrow HAKA that will be revoked during liquidation
    /// @return initialDebtBalance the amount of initial (uUSD) debt the account has
    ...
    function _removeSynth(bytes32 currencyKey) internal {
        ...
        require(currencyKey != uUSD, "Cannot remove synth");

        uint synthSupply = IERC20(synthToRemove).totalSupply();

        if (synthSupply > 0) {
            (uint amountOfuUSD, uint rateToRedeem, ) =
                exchangeRates().effectiveValueAndRates(currencyKey, synthSupply, "uUSD");
            require(rateToRedeem > 0, "Cannot remove without rate");
            ISynthRedeemer _synthRedeemer = synthRedeemer();
            synths[uUSD].issue(address(_synthRedeemer), amountOfuUSD);
            // ensure the debt cache is aware of the new uUSD issued
            debtCache().updateCacheduUSDDebt(SafeCast.toInt256(amountOfuUSD));
    ...
    function issueSynthsWithoutDebt(
        ...
        debtCache().updateCacheduUSDDebt(SafeCast.toInt256(amount.multiplyDecimal(rate)));
     ...
     * @param amount The amount of uUSD collateral to be burnt
     */
    function upgradeCollateralShort(address short, uint amount) external onlyOwner {
        ...
        exchanger().settle(short, uUSD);

        synths[uUSD].burn(short, amount);
    }
    ...
    /// @param isSelfLiquidation boolean to determine if this is a forced or self-invoked liquidation
    /// @return totalRedeemed the total amount of collateral (HAKA) to redeem (liquid and escrow)
    /// @return debtRemoved the amount of debt (uUSD) to burn in order to fix the account's c-ratio
    /// @return escrowToLiquidate the amount of escrow HAKA that will be revoked during liquidation
        ...
        )
    {
        ...
        bool anyRateIsInvalid;
        (debtBalance, , anyRateIsInvalid) = _debtBalanceOfAndTotalDebt(_debtShareBalanceOf(account), uUSD);
        ...
        if (isSelfLiquidation) {
            ...
            // Calculate the amount of debt to remove and the uUSD value of the HAKA required to liquidate.
    ...
    function _issueSynths(
        ...
        // Create their synths
        synths[uUSD].issue(from, amount);

        // Account for the issued debt in the cache
        debtCache().updateCacheduUSDDebt(SafeCast.toInt256(amount));
    }
    ...
    function _burnSynths(
        ...
        // liquidation requires uUSD to be already settled / not in waiting period
        ...
        synths[uUSD].burn(burnAccount, amountBurnt);

        // Account for the burnt debt in the cache.
        debtCache().updateCacheduUSDDebt(-SafeCast.toInt256(amountBurnt));
    }

    // If burning to target, `amount` is ignored, and the correct quantity of uUSD is burnt to reach the target
    ...
    function _voluntaryBurnSynths(
        ...
        if (!burnToTarget) {
            ...
            // First settle anything pending into uUSD as burning or issuing impacts the size of the debt pool
            (, uint refunded, uint numEntriesSettled) = exchanger().settle(from, uUSD);
            if (numEntriesSettled > 0) {
                amount = exchanger().calculateAmountAfterSettlement(from, uUSD, amount, refunded);
            }
        }

        (uint existingDebt, , bool anyRateIsInvalid) = _debtBalanceOfAndTotalDebt(_debtShareBalanceOf(from), uUSD);
