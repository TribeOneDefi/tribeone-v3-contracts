...
// https://docs.tribeone.io/contracts/source/contracts/feepool
contract FeePool is Owned, Proxyable, LimitedSetup, MixinSystemSettings, IFeePool {
    ...
    function _closeSecondary(uint allNetworksSnxBackedDebt, uint allNetworksDebtSharesSupply) internal {
        ...
        // Note: As of SIP-255, all uUSD fee are now automatically burned and are effectively shared amongst stakers in the form of reduced debt.
        if (_recentFeePeriodsStorage(0).feesToDistribute > 0) {
            issuer().burnSynthsWithoutDebt(uUSD, FEE_ADDRESS, _recentFeePeriodsStorage(0).feesToDistribute);
    ...
    /**
     * Note: As of SIP-255, all uUSD fees are burned at the closure of the fee period and are no longer claimable.
    ...
    /**
     * @notice The total fees available in the system to be withdrawn in uUSD.
     */
