...
contract Exchanger is Owned, MixinSystemSettings, IExchanger {
    ...
    function _updateHAKAIssuedDebtOnExchange(bytes32[2] memory currencyKeys, uint[2] memory currencyRates) internal {
        bool includesSUSD = currencyKeys[0] == uUSD || currencyKeys[1] == uUSD;
        ...
        )
    {
        ...
        if (fee > 0) {
            ...
            fee = addrs.exchangeRates.effectiveValue(destinationSettings.currencyKey, fee, uUSD);
    ...
    function _ensureCanExchange(
        ...
        (, bool srcBroken, bool srcStaleOrInvalid) =
            sourceCurrencyKey != uUSD ? exchangeRates().rateWithSafetyChecks(sourceCurrencyKey) : (0, false, false);
    ...
    {
        // no dynamic dynamicFee for uUSD or too few rounds
        if (settings.currencyKey == uUSD || settings.exchangeDynamicFeeRounds <= 1) {
            return (0, false);
        }
        ...
        )
    {
        ...
        require(sourceCurrencyKey == uUSD || !exchangeRates().rateIsInvalid(sourceCurrencyKey), "src synth rate invalid");

        require(
            destinationCurrencyKey == uUSD || !exchangeRates().rateIsInvalid(destinationCurrencyKey),
