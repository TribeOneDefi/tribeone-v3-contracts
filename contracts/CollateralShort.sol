...
contract CollateralShort is Collateral {
    ...
    function open(
        ...
        IERC20(address(_synthuUSD())).transferFrom(msg.sender, address(this), collateral);
    ...
    function close(uint id) external returns (uint amount, uint collateral) {
        ...
        IERC20(address(_synthuUSD())).transfer(msg.sender, collateral);
    }
    ...
    function deposit(
        ...
        require(amount <= IERC20(address(_synthuUSD())).allowance(msg.sender, address(this)), "Allowance too low");

        IERC20(address(_synthuUSD())).transferFrom(msg.sender, address(this), amount);
    ...
    function withdraw(uint id, uint amount) external returns (uint principal, uint collateral) {
        ...
        IERC20(address(_synthuUSD())).transfer(msg.sender, amount);
    }
    ...
    function liquidate(
        ...
        IERC20(address(_synthuUSD())).transfer(msg.sender, collateralLiquidated);
    }
    ...
    function _repayWithCollateral(
        ...
        // 3. Get the equivalent payment amount in uUSD, and also distinguish
        // the fee that would be charged for both principal and interest.
        (uint expectedAmount, uint exchangeFee, ) = _exchanger().getAmountsForExchange(payment, loan.currency, uUSD);
        uint paymentSUSD = expectedAmount.add(exchangeFee);

        // 4. Reduce the collateral by the equivalent (total) payment amount in uUSD,
        ...
        // 5. Pay exchange fees.
        _payFees(exchangeFee, uUSD);

        // 6. Burn uUSD held in the contract.
        _synthuUSD().burn(address(this), collateralToRemove);
