...
contract SynthRedeemer is ISynthRedeemer, MixinResolver {
    ...
    function uUSD() internal view returns (IERC20) {
        return IERC20(requireAndGetAddress(CONTRACT_SYNTHSUSD));
    }

    function totalSupply(IERC20 synthProxy) public view returns (uint supplyInuUSD) {
        supplyInuUSD = synthProxy.totalSupply().multiplyDecimal(redemptions[address(synthProxy)]);
    }

    function balanceOf(IERC20 synthProxy, address account) external view returns (uint balanceInuUSD) {
        balanceInuUSD = synthProxy.balanceOf(account).multiplyDecimal(redemptions[address(synthProxy)]);
    }
    ...
    function _redeem(IERC20 synthProxy, uint amountOfSynth) internal {
        ...
        uint amountInuUSD = amountOfSynth.multiplyDecimal(rateToRedeem);
        uUSD().transfer(msg.sender, amountInuUSD);
        emit SynthRedeemed(address(synthProxy), msg.sender, amountOfSynth, amountInuUSD);
    }
    ...
    function deprecate(IERC20 synthProxy, uint rateToRedeem) external onlyIssuer {
        ...
        uint supplyInuUSD = totalSynthSupply.multiplyDecimal(rateToRedeem);
        require(uUSD().balanceOf(address(this)) >= supplyInuUSD, "uUSD must first be supplied");
        redemptions[synthProxyAddress] = rateToRedeem;
        emit SynthDeprecated(address(synthProxy), rateToRedeem, totalSynthSupply, supplyInuUSD);
    }
    ...
    event SynthRedeemed(address synth, address account, uint amountOfSynth, uint amountInuUSD);
    event SynthDeprecated(address synth, uint rateToRedeem, uint totalSynthSupply, uint supplyInuUSD);
}
