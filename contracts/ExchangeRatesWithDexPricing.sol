...
contract ExchangeRatesWithDexPricing is ExchangeRates {
        ...
        )
    {
        ...
        IDirectIntegrationManager.ParameterIntegrationSettings memory usdSettings =
            directIntegrationManager().getExchangeParameters(msg.sender, uUSD);
    ...
    /// @dev One of `sourceCurrencyKey` or `destCurrencyKey` should be uUSD. There are two parameters to indicate directionality. Because this function returns "price", if the source is uUSD, the result will be flipped.
    ...
    function _getPriceFromDexAggregator(
        ...
        require(
            sourceSettings.currencyKey == uUSD || destinationSettings.currencyKey == uUSD,
            "Atomic swaps must go through uUSD"
        );
        ...
        return destinationSettings.currencyKey == "uUSD" ? result : SafeDecimalMath.unit().divideDecimalRound(result);
    }
    ...
    function synthTooVolatileForAtomicExchange(IDirectIntegrationManager.ParameterIntegrationSettings memory settings)
        ...
        returns (bool)
    {
        // uUSD is a special case and is never volatile
        if (settings.currencyKey == "uUSD") return false;
