...
// https://docs.tribeone.io/contracts/source/contracts/purgeablesynth
contract PurgeableSynth is Synth {
    using SafeDecimalMath for uint;

    // The maximum allowed amount of tokenSupply in equivalent uUSD value for this synth to permit purging
    ...
    function purge(address[] calldata addresses) external optionalProxy_onlyOwner {
        ...
        uint maxSupplyToPurge = exRates.effectiveValue("uUSD", maxSupplyToPurgeInUSD, currencyKey);
        ...
        for (uint i = 0; i < addresses.length; i++) {
            ...
            if (amountHeld > 0) {
                exchanger().exchange(holder, holder, currencyKey, amountHeld, "uUSD", holder, false, address(0), bytes32(0));
