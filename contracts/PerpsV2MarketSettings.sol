...
contract PerpsV2MarketSettings is Owned, MixinPerpsV2MarketSettings, IPerpsV2MarketSettings {
    ...
    /*
     * The minimum amount of uUSD paid to a liquidator when they successfully liquidate a position.
    ...
    /*
     * The maximum amount of uUSD paid to a liquidator when they successfully liquidate a position.
     */
    ...
    function setMinKeeperFee(uint _uUSD) external onlyOwner {
        require(_uUSD <= _minInitialMargin(), "min margin < liquidation fee");
        if (_maxKeeperFee() > 0) {
            // only check if already set
            require(_uUSD <= _maxKeeperFee(), "max fee < min fee");
        }
        _flexibleStorage().setUIntValue(SETTING_CONTRACT_NAME, SETTING_MIN_KEEPER_FEE, _uUSD);
        emit MinKeeperFeeUpdated(_uUSD);
    }

    function setMaxKeeperFee(uint _uUSD) external onlyOwner {
        require(_uUSD >= _minKeeperFee(), "max fee < min fee");
        _flexibleStorage().setUIntValue(SETTING_CONTRACT_NAME, SETTING_MAX_KEEPER_FEE, _uUSD);
        emit MaxKeeperFeeUpdated(_uUSD);
    }
    ...
    event MinKeeperFeeUpdated(uint uUSD);
    event MaxKeeperFeeUpdated(uint uUSD);
