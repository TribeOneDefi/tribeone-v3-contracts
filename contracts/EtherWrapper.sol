...
contract EtherWrapper is Owned, Pausable, MixinResolver, MixinSystemSettings, IEtherWrapper {
    ...
    function totalIssuedSynths() public view returns (uint) {
        ...
        // The uUSD fees are backed by sETH that is withheld during minting and burning.
        return exchangeRates().effectiveValue(sETH, sETHIssued, uUSD).add(uUSDIssued);
    }
    ...
    function distributeFees() external {
        ...
        uint amountSUSD = exchangeRates().effectiveValue(sETH, feesEscrowed, uUSD);
        ...
        // Issue uUSD to the fee pool
        issuer().synths(uUSD).issue(feePool().FEE_ADDRESS(), amountSUSD);
        uUSDIssued = uUSDIssued.add(amountSUSD);
