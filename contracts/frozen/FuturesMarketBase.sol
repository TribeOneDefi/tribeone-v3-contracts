 ...
 * is in profit, then the debt pool pays by issuing uUSD into their margin account,
 * while if the position makes a loss then the debt pool burns uUSD from the margin, reducing the
 ...
 *                                  over all markets, and issues and burns uUSD on each market's behalf.
 *
...
contract FuturesMarketBase is MixinFuturesMarketSettings, IFuturesMarketBaseTypes {
     ...
     * but is at least the _minKeeperFee() of uUSD to prevent underincentivising
     * liquidations of small positions.
     * @param positionSize size of position in fixed point decimal baseAsset units
     * @param price price of single baseAsset unit in uUSD fixed point decimal units
     * @return lFee liquidation fee to be paid to liquidator in uUSD fixed point decimal units
     */
     ...
     * @param positionSize size of position in fixed point decimal baseAsset units
     * @param price price of single baseAsset unit in uUSD fixed point decimal units
     * @return lMargin liquidation margin to maintain in uUSD fixed point decimal units
    ...
    /// Uses the exchanger to get the dynamic fee (SIP-184) for trading from uUSD to baseAsset
    ...
    function _dynamicFeeRate() internal view returns (uint feeRate, bool tooVolatile) {
        return _exchanger().dynamicFeeRateForExchange(uUSD, baseAsset);
    }
    ...
    function _transferMargin(
        ...
        if (marginDelta > 0) {
            ...
            // uUSD balance and credited to their margin account.
            ...
            // their uUSD balance, and debited from their margin account.
     ...
     * withdrawal. The margin will be burnt or issued directly into/out of the caller's uUSD wallet.
     * Reverts on deposit if the caller lacks a sufficient uUSD balance.
