...
contract DebtCache is BaseDebtCache {
    ...
    function updateCacheduUSDDebt(int amount) external onlyIssuer {
        uint delta = SafeDecimalMath.abs(amount);
        if (amount > 0) {
            _cachedSynthDebt[uUSD] = _cachedSynthDebt[uUSD].add(delta);
            _cachedDebt = _cachedDebt.add(delta);
        } else {
            _cachedSynthDebt[uUSD] = _cachedSynthDebt[uUSD].sub(delta);
