...
contract PerpsV2MarketBase is Owned, MixinPerpsV2MarketSettings, IPerpsV2MarketBaseTypes {
     ...
     * but is between _minKeeperFee() and _maxKeeperFee() expressed in uUSD to prevent underincentivising
     ...
     * @return The premium to be paid upon liquidation in uUSD
     */
    ...
    function _dynamicFeeRate() internal view returns (uint feeRate, bool tooVolatile) {
        return _exchanger().dynamicFeeRateForExchange(uUSD, _baseAsset());
    }
