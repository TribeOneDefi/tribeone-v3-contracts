...
contract Depot is Owned, Pausable, ReentrancyGuard, MixinResolver, IDepot {
    ...
    /* This is a convenience variable so users and dApps can just query how much uUSD
    ...
    // The minimum amount of uUSD required to enter the FiFo queue
    uint public minimumDepositAmount = 50 * SafeDecimalMath.unit();

    // A cap on the amount of uUSD you can buy with ETH in 1 transaction
    ...
    bytes32 private constant CONTRACT_SYNTHSUSD = "SynthuUSD";
    ...
    /**
     * @notice Set the minimum deposit amount required to depoist uUSD into the FIFO queue
     * @param _amount The new new minimum number of uUSD required to deposit
     */
    ...
    /**
     * @notice Fallback function (exchanges ETH to uUSD)
     */
    ...
    function exchangeEtherForSynths()
        ...
        notPaused
        returns (
            uint // Returns the number of Synths (uUSD) received
        )
    {
    ...
    function _exchangeEtherForSynths() internal returns (uint) {
        ...
        for (uint i = depositStartIndex; remainingToFulfill > 0 && i < depositEndIndex; i++) {
            ...
            if (deposit.user == address(0)) {
                ...
                if (deposit.amount > remainingToFulfill) {
                    ...
                    synthuUSD().transfer(msg.sender, remainingToFulfill);
                    ...
                    //       we request a specific transfer here, the fee is
                    //       automatically deducted and sent to the fee pool.
                    synthuUSD().transfer(msg.sender, deposit.amount);
        ...
        if (fulfilled > 0) {
            ...
            emit Exchange("ETH", msg.value, "uUSD", fulfilled);
        }
    ...
    /**
     * @notice Exchange ETH to uUSD while insisting on a particular rate. This allows a user to
    ...
    function _exchangeSynthsForHAKA(uint synthAmount) internal returns (uint) {
        ...
        synthuUSD().transferFrom(msg.sender, fundsWallet, synthAmount);

        // And send them the HAKA.
        tribeone().transfer(msg.sender, tribeoneToSend);

        emit Exchange("uUSD", synthAmount, "HAKA", tribeoneToSend);

        return tribeoneToSend;
    }

    /**
     * @notice Exchange uUSD for HAKA
    ...
    /**
     * @notice Exchange uUSD for HAKA while insisting on a particular rate. This allows a user to
     *         exchange while protecting against frontrunning by the contract owner on the exchange rate.
     * @param synthAmount The amount of synths the user wishes to exchange.
    ...
    function withdrawMyDepositedSynths() external {
        ...
        synthuUSD().transfer(msg.sender, synthsToSend);
     ...
     * @param amount The amount of uUSD you wish to deposit (must have been approved first)
     */
    function depositSynths(uint amount) external {
        // Grab the amount of synths. Will fail if not approved first
        synthuUSD().transferFrom(msg.sender, address(this), amount);
    ...
    function tribeoneReceivedForEther(uint amount) public view returns (uint) {
        // How much is the ETH they sent us worth in uUSD (ignoring the transfer fee)?
    ...
    function synthuUSD() internal view returns (IERC20) {
