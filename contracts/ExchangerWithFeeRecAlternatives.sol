...
contract ExchangerWithFeeRecAlternatives is MinimalProxyFactory, Exchanger {
        ...
        )
    {
        ...
        IDirectIntegrationManager.ParameterIntegrationSettings memory destinationSettings =
            _exchangeSettings(msg.sender, destinationCurrencyKey);
        IDirectIntegrationManager.ParameterIntegrationSettings memory usdSettings = _exchangeSettings(msg.sender, uUSD);
    ...
    function _exchangeAtomically(
...

        {
            ...
            IDirectIntegrationManager.ParameterIntegrationSettings memory usdSettings = _exchangeSettings(from, uUSD);
            ...
            // Determine uUSD value of exchange
            uint sourceSusdValue;
            if (sourceCurrencyKey == uUSD) {
                // Use after-settled amount as this is amount converted (not sourceAmount)
                sourceSusdValue = sourceAmountAfterSettlement;
            } else if (destinationCurrencyKey == uUSD) {
                // In this case the systemConvertedAmount would be the fee-free uUSD value of the source synth
                sourceSusdValue = systemConvertedAmount;
            } else {
                // Otherwise, convert source to uUSD value
