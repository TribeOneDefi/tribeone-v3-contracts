...
describe('publish scripts', () => {
	...
	beforeEach(async () => {
		...
		[uUSD, sBTC, sETH] = ['uUSD', 'sBTC', 'sETH'].map(toBytes32);
	...
	describe('integrated actions test', () => {
		describe('when deployed', () => {
			...
			beforeEach(async () => {
				...
				sources = getSource();
				targets = getTarget();
				synths = getSynths().filter(({ name }) => name !== 'uUSD');
				...
				uUSDContract = getContract({ target: 'ProxyuUSD', source: 'Synth' });
			...
			describe('default system settings', () => {
				...
				describe('when defaults are changed', () => {
					...
					beforeEach(async () => {
						...
						newRateForuUSD = ethers.utils.parseEther('0.1').toString();
					...
					describe('when redeployed with a new system settings contract', () => {
						...
						it('then the defaults remain unchanged', async () => {
							...
							assert.strictEqual(
								(
									await Exchanger.feeRateForExchange(toBytes32('(ignored)'), toBytes32('uUSD'))
								).toString(),
								newRateForuUSD
							);
						});
			...
			describe('synths added to Issuer', () => {
				...
				describe('when only uUSD and sETH is chosen as a synth', () => {
					beforeEach(async () => {
						fs.writeFileSync(
							synthsJSONPath,
							JSON.stringify([
					...
					describe('when Issuer redeployed', () => {
						...
						it('then only uUSD is added to the issuer', async () => {
							const keys = await Issuer.availableCurrencyKeys();
							assert.deepStrictEqual(keys.map(hexToString), ['uUSD', 'sETH']);
						});
					});
				});
			});
			...
			describe('when ExchangeRates has prices HAKA $0.30 and all synths $1', () => {
				...
				describe('when transferring 100k HAKA to user1', () => {
					...
					describe('when user1 issues all possible uUSD', () => {
						beforeEach(async () => {
						...
						it('then the uUSD balanced must be 100k * 0.3 * 0.2 (default SystemSettings.issuanceRatio) = 6000', async () => {
							const balance = await callMethodWithRetry(
								uUSDContract.balanceOf(accounts.first.address)
							);
							assert.strictEqual(
						...
						describe('when user1 exchange 1000 uUSD for sETH (the MultiCollateralSynth)', () => {
							let sETHBalanceAfterExchange;
							beforeEach(async () => {
								await TribeOne.exchange(uUSD, ethers.utils.parseEther('1000'), sETH, overrides);
								sETHBalanceAfterExchange = await callMethodWithRetry(
									sETHContract.balanceOf(accounts.first.address)
								);
							});
							it('then their uUSD balance is 5000', async () => {
								const balance = await callMethodWithRetry(
									uUSDContract.balanceOf(accounts.first.address)
								);
								assert.strictEqual(
							...
							it('and their sETH balance is 1000 - the fee', async () => {
								const { amountReceived } = await callMethodWithRetry(
									Exchanger.getAmountsForExchange(ethers.utils.parseEther('1000'), uUSD, sETH)
								);
								assert.strictEqual(
						...
						describe('when user1 exchange 1000 uUSD for sBTC', () => {
							...
							it('and their sBTC balance is 1000 - the fee', async () => {
								const { amountReceived } = await callMethodWithRetry(
									Exchanger.getAmountsForExchange(ethers.utils.parseEther('1000'), uUSD, sBTC)
								);
								assert.strictEqual(
							...
							describe('when user1 burns 10 uUSD', () => {
								beforeEach(async () => {
									let tx;
								...
								it('then their uUSD balance is 4990', async () => {
									const balance = await callMethodWithRetry(
										uUSDContract.balanceOf(accounts.first.address)
									);
									assert.strictEqual(
								...
								describe('when deployer replaces sBTC with PurgeableSynth', () => {
									...
									describe('and deployer invokes purge', () => {
										...
										it('then their uUSD balance is 4990 + sBTCBalanceAfterExchange', async () => {
											const balance = await callMethodWithRetry(
												uUSDContract.balanceOf(accounts.first.address)
											);
											const [amountReceived] = await callMethodWithRetry(
												Exchanger.getAmountsForExchange(sBTCBalanceAfterExchange, sBTC, uUSD)
											);
											assert.strictEqual(
												ethers.utils.formatEther(balance.toString()),
