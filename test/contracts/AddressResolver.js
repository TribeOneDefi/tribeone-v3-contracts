	...
	describe('getSynth()', () => {
		describe('when a mock for Issuer is added', () => {
			...
			it('when getSynth() is invoked', async () => {
				const synth = await resolver.getSynth(toBytes32('uUSD'));
		...
		describe('when a TribeOne is created with a few added synths', () => {
			...
			let uUSDContract;
			beforeEach(async () => {
				({ SynthsETH: sETHContract, SynthuUSD: uUSDContract } = await setupAllContracts({
					accounts,
					existing: {
			...
			it('when getSynth() is invoked with these synth keys, they are returned correctly', async () => {
				assert.equal(await resolver.getSynth(toBytes32('uUSD')), uUSDContract.address);
