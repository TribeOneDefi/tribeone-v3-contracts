...
contract('BaseTribeone', async accounts => {
	const [hUSD, sAUD, sEUR, HAKA, hETH] = ['hUSD', 'sAUD', 'sEUR', 'HAKA', 'hETH'].map(toBytes32);
	...
	describe('constructor', () => {
		...
		it('should set constructor params on upgrade to new totalSupply', async () => {
			const YEAR_2_TRIBEONEETIX_TOTAL_SUPPLY = web3.utils.toWei('175000000');
			const instance = await setupContract({
				contract: 'BaseTribeone',
				accounts,
				skipPostDeploy: true,
				args: [account1, account2, owner, YEAR_2_TRIBEONEETIX_TOTAL_SUPPLY, addressResolver.address],
			});

			assert.equal(await instance.proxy(), account1);
			assert.equal(await instance.tokenState(), account2);
			assert.equal(await instance.owner(), owner);
			assert.equal(await instance.totalSupply(), YEAR_2_TRIBEONEETIX_TOTAL_SUPPLY);
			assert.equal(await instance.resolver(), addressResolver.address);
		});
	});
	...
	describe('only Exchanger can call emit event functions', () => {
		...
		it('emitTribeExchange() cannot be invoked directly by any account', async () => {
			await onlyGivenAddressCanInvoke({
				fnc: baseTribeoneImpl.emitTribeExchange,
				accounts,
		...
		describe('Exchanger calls emit', () => {
			...
			beforeEach('call event emission functions', async () => {
				...
				tx3 = await baseTribeoneImpl.emitTribeExchange(
					account1,
					currencyKey1,
					amount1,
					currencyKey2,
	...
	describe('isWaitingPeriod()', () => {
		it('returns false by default', async () => {
			assert.isFalse(await baseTribeoneImpl.isWaitingPeriod(hETH));
		});
		describe('when a user has exchanged into hETH', () => {
			beforeEach(async () => {
				await updateRatesWithDefaults({ exchangeRates, owner, debtCache });

				await baseTribeoneImpl.issueTribes(toUnit('100'), { from: owner });
				await baseTribeoneImpl.exchange(hUSD, toUnit('10'), hETH, { from: owner });
			});
			it('then waiting period is true', async () => {
				assert.isTrue(await baseTribeoneImpl.isWaitingPeriod(hETH));
			});
	...
	describe('anyTribeOrHAKARateIsInvalid()', () => {
		it('should have stale rates initially', async () => {
			assert.equal(await baseTribeoneImpl.anyTribeOrHAKARateIsInvalid(), true);
		});
		describe('when tribe rates set', () => {
			...
			describe('when HAKA is also set', () => {
				...
				it('then no stale rates', async () => {
					assert.equal(await baseTribeoneImpl.anyTribeOrHAKARateIsInvalid(), false);
				});
	...
	describe('availableCurrencyKeys()', () => {
		it('returns all currency keys by default', async () => {
			assert.deepEqual(await baseTribeoneImpl.availableCurrencyKeys(), [hUSD, hETH, sEUR, sAUD]);
		});
	});
	...
	describe('transfer()', () => {
		...
		it('should revert when exceeding locked tribeone and calling the ERC20 transfer function', async () => {
			...
			// Issue max tribes.
			await baseTribeoneImpl.issueMaxTribes({ from: owner });
		...
		describe('rates stale for transfers', () => {
			...
			describe('when the user has a debt position', () => {
				beforeEach(async () => {
					...
					await Promise.all([
						baseTribeoneImpl.issueTribes(toUnit('1'), { from: account1 }),
						baseTribeoneImpl.issueTribes(toUnit('1'), { from: account2 }),
					]);
		...
		describe('when the user holds HAKA', () => {
			...
			describe('and has an escrow entry', () => {
				...
				describe('when the user has a debt position (i.e. has issued)', () => {
					beforeEach(async () => {
						await baseTribeoneImpl.issueTribes(toUnit('10'), { from: account1 });
					});
		...
		it('should not be possible to transfer locked tribeone', async () => {
			...
			await baseTribeoneImpl.issueTribes(amountIssued, { from: account1 });
		...
		it("should lock newly received tribeone if the user's collaterisation is too high", async () => {
			...
			const maxIssuableTribes = await baseTribeoneImpl.maxIssuableTribes(account1);

			// Issue
			await baseTribeoneImpl.issueTribes(maxIssuableTribes, { from: account1 });

			// Exchange into sEUR
			await baseTribeoneImpl.exchange(hUSD, maxIssuableTribes, sEUR, { from: account1 });
		...
		it('should unlock tribeone when collaterisation ratio changes', async () => {
			...
			// Issue
			const issuedTribes = await baseTribeoneImpl.maxIssuableTribes(account1);
			await baseTribeoneImpl.issueTribes(issuedTribes, { from: account1 });
			const remainingIssuable = (await baseTribeoneImpl.remainingIssuableTribes(account1))[0];
			...
			// Exchange into sAUD
			await baseTribeoneImpl.exchange(hUSD, issuedTribes, sAUD, { from: account1 });
