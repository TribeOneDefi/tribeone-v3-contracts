...
contract('Wrapper', async accounts => {
	const synths = ['uUSD', 'sETH', 'ETH', 'HAKA'];
	const [sETH, uUSD, ETH] = ['sETH', 'uUSD', 'ETH'].map(toBytes32);
	...
	const calculateETHToUSD = async feesInETH => {
		// Ask the Depot how many uUSD I will get for this ETH
		const expectedFeeuUSD = await depot.synthsReceivedForEther(feesInETH);
		return expectedFeeuUSD;
	};

	const calculateMintFees = async amount => {
		const mintFee = (await etherWrapper.calculateMintFee(amount))[0];
		const expectedFeeuUSD = await calculateETHToUSD(mintFee);
		return { mintFee, expectedFeeuUSD };
	};

	const calculateBurnFees = async amount => {
		const burnFee = (await etherWrapper.calculateBurnFee(amount))[0];
		const expectedFeeuUSD = await calculateETHToUSD(burnFee);
		return { burnFee, expectedFeeuUSD };
	};
	...
	describe('totalIssuedSynths', async () => {
		...
		before(async () => {
			feeAmount = await exchangeRates.effectiveValue(sETH, toUnit('0.005'), uUSD);
		});
		...
		describe('when amount is less than than capacity', () => {
			...
			describe('fee is positive', () => {
				beforeEach(async () => {
					...
					({ mintFee, expectedFeeuUSD } = await calculateMintFees(amount));
				...
				it('escrows `amount * mintFeeRate` worth of uUSD as fees', async () => {
					assert.bnEqual(await wrapperFactory.feesEscrowed(), feesEscrowed.add(expectedFeeuUSD));
				});
		...
		describe('amount is larger than or equal to capacity', () => {
			...
			describe('fee is positive', () => {
				beforeEach(async () => {
					...
					({ mintFee, expectedFeeuUSD } = await calculateMintFees(initialCapacity));
				...
				it('escrows `capacity * mintFeeRate` worth of uUSD as fees', async () => {
					assert.bnEqual(await wrapperFactory.feesEscrowed(), feesEscrowed.add(expectedFeeuUSD));
				});
			...
			describe('when amount is strictly lower than reserves(1+burnFeeRate)', async () => {
				...
				describe('fee is positive', () => {
					beforeEach(async () => {
						...
						({ burnFee, expectedFeeuUSD } = await calculateBurnFees(principal));
			...
			describe('when amount is larger than or equal to reserves(1+burnFeeRate)', async () => {
				...
				describe('fee is positive', () => {
					beforeEach(async () => {
						...
						({ burnFee, expectedFeeuUSD } = await calculateBurnFees(reserves));
					...
					it('escrows `amount * burnFeeRate` worth of uUSD as fees', async () => {
						assert.bnEqual(await wrapperFactory.feesEscrowed(), feesEscrowed.add(expectedFeeuUSD));
					});
