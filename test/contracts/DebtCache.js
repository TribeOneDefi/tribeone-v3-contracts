...
contract('DebtCache', async accounts => {
	const [uUSD, sAUD, sEUR, HAKA, sETH, ETH, iETH] = [
		'uUSD',
		'sAUD',
		'sEUR',
		'HAKA',
	...
	const setupMultiCollateral = async () => {
		...
		await ceth.addSynths(
			['SynthuUSD', 'SynthsETH'].map(toBytes32),
			['uUSD', 'sETH'].map(toBytes32),
			{ from: owner }
		);
	...
	const setupShort = async () => {
		...
		await uUSDContract.approve(short.address, toUnit(100000), { from: account1 });
	};
	...
	before(async () => {
		synths = ['uUSD', 'sAUD', 'sEUR', 'sETH', 'iETH'];
		({
			TribeOne: tribeone,
	...
	describe('protected methods', () => {
		...
		it('updateCacheduUSDDebt() can only be invoked by the issuer', async () => {
			await onlyGivenAddressCanInvoke({
				fnc: debtCache.updateCacheduUSDDebt,
				args: [toUnit('1')],
				accounts,
				reason: 'Sender is not Issuer',
			});
	...
	describe('After issuing synths', () => {
		beforeEach(async () => {
			...
			// Issue 1000 uUSD worth of tokens to a user
			await uUSDContract.issue(account1, toUnit(100));
		...
		describe('Current issued debt', () => {
			...
			it('Live debt is reported accurately for individual currencies', async () => {
				const result = await debtCache.currentSynthDebts([uUSD, sEUR, sAUD, sETH]);
		...
		describe('takeDebtSnapshot()', () => {
			...
			it('updates the cached values for all individual synths', async () => {
				...
				await debtCache.takeDebtSnapshot();
				let debts = await debtCache.currentSynthDebts([uUSD, sEUR, sAUD, sETH]);
				...
				debts = await debtCache.cachedSynthDebts([uUSD, sEUR, sAUD, sETH]);
				assert.bnEqual(debts[0], toUnit(100));
			...
			describe('properly incorporates futures market debt', () => {
				it('when no market exist', async () => {
					...
					uUSDContract.issue(account1, toUnit(100), { from: owner });
					await debtCache.takeDebtSnapshot();
		...
		describe('updateCachedSynthDebts()', () => {
			it('allows resynchronisation of subsets of synths', async () => {
				...
				assert.bnEqual(await issuer.totalIssuedSynths(uUSD, true), toUnit(600));
				...
				assert.bnEqual(await issuer.totalIssuedSynths(uUSD, true), toUnit(900));
			...
			it('reverts when attempting to synchronise non-existent synths or HAKA', async () => {
				await assert.revert(debtCache.updateCachedSynthDebts([HAKA]));
				const fakeSynth = toBytes32('FAKE');
				await assert.revert(debtCache.updateCachedSynthDebts([fakeSynth]));
				await assert.revert(debtCache.updateCachedSynthDebts([uUSD, fakeSynth]));
			});
		...
		describe('updateCacheduUSDDebt()', () => {
			...
			it('when uUSD is increased by minting', async () => {
				const cachedSynthDebt = (await debtCache.cachedSynthDebts([uUSD]))[0];
				const amount = toUnit('1000');
				const tx = await debtCache.updateCacheduUSDDebt(amount, { from: owner });

				assert.bnEqual((await debtCache.cacheInfo())[0], cachedSynthDebt.add(amount));
				assert.bnEqual(await debtCache.cachedSynthDebts([uUSD]), cachedSynthDebt.add(amount));
			...
			it('when uUSD cache is decreased by minting', async () => {
				const amount = toUnit('1000');
				await debtCache.updateCacheduUSDDebt(amount, { from: owner });

				// cached Synth after increase
				const cachedSynthDebt = (await debtCache.cachedSynthDebts([uUSD]))[0];
				assert.bnEqual((await debtCache.cacheInfo())[0], amount);
				assert.bnEqual(await debtCache.cachedSynthDebts([uUSD]), amount);

				// decrease the cached uUSD amount
				const amountToReduce = toUnit('500');
				const tx = await debtCache.updateCacheduUSDDebt(amountToReduce.neg(), { from: owner });

				assert.bnEqual(
					await debtCache.cachedSynthDebts([uUSD]),
		...
		describe('Issuance, burning, exchange, settlement', () => {
			it('issuing uUSD updates the debt total', async () => {
				await debtCache.takeDebtSnapshot();
				...
				const tx = await tribeone.issueSynths(synthsToIssue, { from: account1 });
			...
			it('burning uUSD updates the debt total', async () => {
				await debtCache.takeDebtSnapshot();
				const synthsToIssue = toUnit('10');
				await tribeone.transfer(account1, toUnit('1000'), { from: owner });
				await tribeone.issueSynths(synthsToIssue, { from: account1 });
				...
				const tx = await tribeone.burnSynths(synthsToBurn, { from: account1 });
			...
			it('issuing uUSD updates the total debt cached and uUSD cache', async () => {
				await debtCache.takeDebtSnapshot();
				const issued = (await debtCache.cacheInfo())[0];

				const synthsToIssue = toUnit('1000');
				const cachedSynths = (await debtCache.cachedSynthDebts([uUSD]))[0];
				...
				// cached uUSD increased by synth issued
				assert.bnEqual(await debtCache.cachedSynthDebts([uUSD]), cachedSynths.add(synthsToIssue));
				assert.bnEqual((await debtCache.cacheInfo())[0], issued.add(synthsToIssue));
			});

			it('burning uUSD reduces the total debt and uUSD cache', async () => {
				await debtCache.takeDebtSnapshot();
				...
				// cached uUSD decreased by synth burned
				assert.bnEqual(await debtCache.cachedSynthDebts([uUSD]), cachedSynths.sub(synthsToBurn));
				assert.bnEqual((await debtCache.cacheInfo())[0], issued.sub(synthsToBurn));
			});
			...
			it('exchanging between synths updates the debt totals for those synths', async () => {
				...
				await systemSettings.setExchangeFeeRateForSynths([sAUD, uUSD], [toUnit(0), toUnit(0)], {
					from: owner,
				});
				...
				const issued = (await debtCache.cacheInfo())[0];
				const debts = await debtCache.cachedSynthDebts([uUSD, sAUD]);
				const tx = await tribeone.exchange(uUSD, toUnit('5'), sAUD, { from: account1 });
				const postDebts = await debtCache.cachedSynthDebts([uUSD, sAUD]);
			...
			it('exchanging between synths updates uUSD debt total due to fees', async () => {
				...
				const debts = await debtCache.cachedSynthDebts([uUSD, sAUD, sEUR]);

				await tribeone.exchange(sEUR, toUnit(10), sAUD, { from: account1 });
				const postDebts = await debtCache.cachedSynthDebts([uUSD, sAUD, sEUR]);
			...
			it('settlement updates debt totals', async () => {
				...
				// so there's now 100 - 25 uUSD left (25 of it was exchanged)
	...
	describe('totalNonSnxBackedDebt', async () => {
		...
		describe('when MultiCollateral loans are opened', async () => {
			...
			describe('after the synths are exchanged into other synths', async () => {
				...
				beforeEach(async () => {
					...
					tx = await tribeone.exchange(sETH, '5', sAUD, { from: account1 });
				});
		...
		describe('when shorts are opened', async () => {
			...
			beforeEach(async () => {
				...
				// uUSD collateral = 1.5 * rate_eth
				amount = multiplyDecimalRound(rate, toUnit('1.5'));
				await uUSDContract.issue(account1, amount, { from: owner });
