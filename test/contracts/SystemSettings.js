...
contract('SystemSettings', async accounts => {
	...
	describe('setExchangeFeeRateForSynths()', () => {
		describe('Given synth exchange fee rates to set', async () => {
			const [uUSD, sETH, sAUD, sBTC] = ['uUSD', 'sETH', 'sAUD', 'sBTC'].map(toBytes32);
			...
			it('when input array lengths dont match then revert ', async () => {
				await assert.revert(
					systemSettings.setExchangeFeeRateForSynths([uUSD, sAUD], [toUnit('0.1')], {
						from: owner,
					}),
			...
			it('when owner sets an exchange fee rate larger than MAX_EXCHANGE_FEE_RATE then revert', async () => {
				await assert.revert(
					systemSettings.setExchangeFeeRateForSynths([uUSD], [toUnit('11')], {
						from: owner,
					}),
			...
			describe('Given new synth exchange fee rates to store', async () => {
				it('when 1 exchange rate then store it to be readable', async () => {
					await systemSettings.setExchangeFeeRateForSynths([uUSD], [fxBIPS], {
						from: owner,
					});
					let uUSDRate = await systemSettings.exchangeFeeRate(uUSD);
					assert.bnEqual(uUSDRate, fxBIPS);

					uUSDRate = await systemSettings.exchangeFeeRate(uUSD);
					assert.bnEqual(uUSDRate, fxBIPS);
				});
				it('when 1 exchange rate then emits update event', async () => {
					const transaction = await systemSettings.setExchangeFeeRateForSynths([uUSD], [fxBIPS], {
						from: owner,
					});
				...
				it('when multiple exchange rates then store them to be readable', async () => {
					...
					const uUSDRate = await systemSettings.exchangeFeeRate(uUSD);
					assert.bnEqual(uUSDRate, fxBIPS);
	...
	describe('setPureChainlinkPriceForAtomicSwapsEnabled()', () => {
		const currencyKey = toBytes32('uUSD');
		it('only owner can invoke', async () => {
			await onlyGivenAddressCanInvoke({
