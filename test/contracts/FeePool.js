...
contract('FeePool', async accounts => {
	...
	// CURRENCIES
	const [uUSD, sAUD, HAKA] = ['uUSD', 'sAUD', 'HAKA'].map(toBytes32);
	...
	describe('when the issuanceRatio is 0.2', () => {
		...
		it('should track fee withdrawals correctly', async () => {
			...
			// Issue uUSD for two different accounts.
			await tribeone.transfer(account1, toUnit('1000000'), {
				from: owner,
			});
			...
			await tribeone.exchange(uUSD, exchange, sAUD, { from: owner });
		...
		it('should correctly calculate the totalFeesAvailable for a single open period', async () => {
			...
			// Generate a fee.
			await tribeone.exchange(uUSD, amount, sAUD, { from: owner });
		...
		it('should correctly calculate the totalFeesAvailable and totalFeesBurned for multiple periods', async () => {
			...
			// Generate a fee.
			await tribeone.exchange(uUSD, amount1, sAUD, { from: owner });
			...
			// Generate a fee.
			await tribeone.exchange(uUSD, amount2, sAUD, { from: account1 });
		...
		it('should correctly calculate the feesAvailable for a single user in multiple periods when fees are partially claimed', async () => {
			...
			// As of SIP-255, all uUSD fees are now burned when the fee period closes and are considered claimed, so they should have zero available.
			feesAvailable = await feePool.feesAvailable(account1);
		...
		describe('when closing the fee period', () => {
			...
			it('should import feePeriods and close the current fee period correctly', async () => {
				...
				await uUSDContract.transfer(FEE_ADDRESS, toUnit('1000'), {
					from: owner,
				});
			...
			it('should correctly roll over unclaimed fees when closing fee periods', async () => {
				...
				await uUSDContract.transfer(account1, toUnit('10000'), {
					from: owner,
				});

				// Assert that the correct fee is in the fee pool.
				const fee = await uUSDContract.balanceOf(FEE_ADDRESS);
			...
			it('should receive fees from WrapperFactory', async () => {
				// Make sure some debt exists otherwise updateCacheduUSDDebt will revert when closing/burning fees.
				await tribeone.issueSynths(toUnit('1000'), { from: owner });

				// Close the current one so we know exactly what we're dealing with
				await closeFeePeriod();

				// Wrapper Factory collects 100 uUSD in fees
				const collectedFees = toUnit(100);
				await uUSDContract.issue(wrapperFactory.address, collectedFees);
		...
		describe('claimFees()', () => {
			describe('potential blocking conditions', () => {
				beforeEach(async () => {
					...
					await tribeone.issueSynths(toUnit('10000'), { from: owner });
					await tribeone.exchange(uUSD, toUnit('10'), sAUD, { from: owner });
					await closeFeePeriod();
				});
				['System', 'Issuance'].forEach(section => {
					describe(`when ${section} is suspended`, () => {
			...
			it('should allow a user to claim their fees in uUSD @gasprofile', async () => {
				const length = (await feePool.FEE_PERIOD_LENGTH()).toNumber();

				// Issue 10,000 uUSD for two different accounts.
				await tribeone.transfer(account1, toUnit('1000000'), {
					from: owner,
				});
				...
				for (let i = 0; i <= length; i++) {
					...
					await tribeone.exchange(uUSD, exchange1, sAUD, { from: owner });
					await tribeone.exchange(uUSD, exchange2, sAUD, { from: account1 });
				...
				const beforeUSDBalance = await uUSDContract.balanceOf(owner);
				...
				assert.eventEqual(claimFeesTx, 'FeesClaimed', {
					uUSDAmount: feesBurnedUSD,
					snxRewards: feesAvailableUSD[1],
				});

				const afterUSDBalance = await uUSDContract.balanceOf(owner);
				// uUSD balance should remain unchanged
				assert.bnEqual(afterUSDBalance, beforeUSDBalance);
			});

			it('should allow a user to claim their fees in uUSD after burning @gasprofile', async () => {
				// Issue 10,000 uUSD for two different accounts.
				await tribeone.transfer(account1, toUnit('1000000'), {
					from: owner,
				});
				...
				await tribeone.exchange(uUSD, toUnit(100), sAUD, { from: account1 });
				...
				assert.bnEqual(
					await tribeone.debtBalanceOf(owner, toBytes32('uUSD')),
					toUnit('0'),
			...
			it('should allow a user to claim their fees if they minted debt during period', async () => {
				...
				const oldSynthBalance = await uUSDContract.balanceOf(owner);

				// Now we should be able to claim them.
				await feePool.claimFees({ from: owner });

				// Balance remains the same since fees are burned
				assert.bnEqual(await uUSDContract.balanceOf(owner), oldSynthBalance);
			...
			it('should allow a user to claim their fees in uUSD (as half of total) after some exchanging', async () => {
				const length = (await feePool.FEE_PERIOD_LENGTH()).toNumber();
				...
				assert.bnClose(
					await uUSDContract.balanceOf(owner),
		...
		describe('FeeClaimablePenaltyThreshold', async () => {
			...
			it('should revert when users try to claim fees with > 10% of threshold', async () => {
				...
				const amount = await uUSDContract.balanceOf(account1);
				await tribeone.issueSynths(amount, { from: owner });
				await closeFeePeriod();
		...
		describe('claimOnBehalf', async () => {
			async function generateFees() {
				...
				// generate fee
				await tribeone.exchange(uUSD, exchange1, sAUD, { from: account1 });
			...
			it('should approve a claim on behalf for account1 by account2 and have fees in wallet', async () => {
				...
				const oldSynthBalance = await uUSDContract.balanceOf(account1);
				...
				assert.bnClose(
					await uUSDContract.balanceOf(account1),
					oldSynthBalance.add(feesAvailable[0]),
					'250000000000000000'
				);
			});
