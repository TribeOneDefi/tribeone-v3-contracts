...
contract('Liquidator', accounts => {
	const [uUSD, HAKA] = ['uUSD', 'HAKA'].map(toBytes32);
	...
	const setLiquidHAKABalance = async (account, amount) => {
		// burn debt
		await tribeone.burnSynths(await synthuUSD.balanceOf(account), { from: account });
		// remove all snx
		...
		describe('calculateAmountToFixCollateral', () => {
			...
			describe('given target ratio of 800%, collateral of $600, debt of $300', () => {
				...
				describe('given liquidation penalty is 10%', () => {
					it('calculates uUSD to fix ratio from 200%, with $600 HAKA collateral and $300 debt', async () => {
					...
					it('calculates uUSD to fix ratio from 300%, with $600 HAKA collateral and $200 debt', async () => {
			...
			describe('when Alice is undercollateralized', () => {
				...
				describe('given Alice has $600 Debt, $800 worth of HAKA Collateral and c-ratio at 133.33%', () => {
					describe('when Alice calls self liquidate', () => {
						...
						beforeEach(async () => {
							...
							aliceDebtValueBefore = await tribeone.debtBalanceOf(alice, uUSD);

							// Record Bobs state
							bobDebtValueBefore = await tribeone.debtBalanceOf(bob, uUSD);
						...
						it('it succeeds and the ratio is fixed', async () => {
							...
							const bobDebtValueAfter = await tribeone.debtBalanceOf(bob, uUSD);
				...
				describe('with some HAKA in escrow', () => {
					...
					it('escrow balance is not used for self-liquidation', async () => {
						...
						const debtBefore = await tribeone.debtBalanceOf(alice, uUSD);
						const totalDebt = await tribeone.totalIssuedSynths(uUSD);
						...
						assert.bnEqual(await tribeone.totalIssuedSynths(uUSD), totalDebt);
						...
						assert.bnClose(
							await tribeone.debtBalanceOf(alice, uUSD),
						...
						describe('when Alice does not fix her c-ratio ', () => {
							...
							describe('with only escrowed HAKA', () => {
								...
								it('escrow balance is used for liquidation (partial)', async () => {
									...
									const debtAfter = await tribeone.debtBalanceOf(alice, uUSD);
