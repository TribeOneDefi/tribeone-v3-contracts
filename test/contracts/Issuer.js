...
contract('Issuer (via TribeOne)', async accounts => {
	...
	const [uUSD, sAUD, sEUR, HAKA, sETH, ETH] = ['uUSD', 'sAUD', 'sEUR', 'HAKA', 'sETH', 'ETH'].map(
		toBytes32
	);
	const synthKeys = [uUSD, sAUD, sEUR, sETH, HAKA];
	...
	before(async () => {
		...
		({
			...
			contracts: [
				...
				'DebtCache',
				'Exchanger', // necessary for burnSynths to check settlement of uUSD
		...
		describe('when the issuanceRatio is 0.2', () => {
				...
				describe('when exchange rates set', () => {
					...
					describe('when numerous issues in many currencies', () => {
						beforeEach(async () => {
							...
							await uUSDContract.issue(account1, toUnit('1000'));

							await sAUDContract.issue(account1, toUnit('1000')); // 500 uUSD worth
							await sAUDContract.issue(account2, toUnit('1000')); // 500 uUSD worth

							await sEURContract.issue(account3, toUnit('80')); // 100 uUSD worth

							await sETHContract.issue(account1, toUnit('1')); // 100 uUSD worth

							// and since we are are bypassing the usual issuance flow here, we must cache the debt snapshot
							assert.bnEqual(await tribeone.totalIssuedSynths(uUSD), toUnit('0'));
							await updateDebtMonitors();
						});
				...
				describe('when exchange rates set', () => {
					...
					describe('when numerous issues in one currency', () => {
						beforeEach(async () => {
							// as our synths are mocks, let's issue some amount to users
							await uUSDContract.issue(account1, toUnit('1000'));
							await uUSDContract.issue(account2, toUnit('100'));
							await uUSDContract.issue(account3, toUnit('10'));
							await uUSDContract.issue(account1, toUnit('1'));

							// and since we are are bypassing the usual issuance flow here, we must cache the debt snapshot
							assert.bnEqual(await tribeone.totalIssuedSynths(uUSD), toUnit('0'));
							await updateDebtMonitors();
						});
						it('then totalIssuedSynths in should correctly calculate the total issued synths in uUSD', async () => {
							assert.bnEqual(await tribeone.totalIssuedSynths(uUSD), toUnit('1111'));
						});
					...
					describe('when numerous issues in many currencies', () => {
						...
						it('then totalIssuedSynths in should correctly calculate the total issued synths in uUSD', async () => {
							assert.bnEqual(await tribeone.totalIssuedSynths(uUSD), toUnit('2200'));
						});
						it('and in another synth currency', async () => {
			...
			describe('debtBalance()', () => {
				it('should not change debt balance % if exchange rates change', async () => {
					...
					await tribeone.exchange(uUSD, amountIssuedAcc2, sAUD, { from: account2 });

					const PRECISE_UNIT = web3.utils.toWei(web3.utils.toBN('1'), 'gether');
					let totalIssuedSynthuUSD = await tribeone.totalIssuedSynths(uUSD);
					const account1DebtRatio = divideDecimal(
						amountIssuedAcc1,
						totalIssuedSynthuUSD,
						PRECISE_UNIT
					);
					...
					totalIssuedSynthuUSD = await tribeone.totalIssuedSynths(uUSD);
					const conversionFactor = web3.utils.toBN(1000000000);
					const expectedDebtAccount1 = multiplyDecimal(
						account1DebtRatio,
						totalIssuedSynthuUSD.mul(conversionFactor),
						PRECISE_UNIT
					...
					assert.bnClose(await tribeone.debtBalanceOf(account1, uUSD), expectedDebtAccount1);
					assert.bnClose(await tribeone.debtBalanceOf(account2, uUSD), expectedDebtAccount2);
				});
				...
				it("should correctly calculate a user's debt balance without prior issuance", async () => {
					...
					const debt1 = await tribeone.debtBalanceOf(account1, toBytes32('uUSD'));
					const debt2 = await tribeone.debtBalanceOf(account2, toBytes32('uUSD'));
				...
				it("should correctly calculate a user's debt balance with prior issuance", async () => {
					...
					const debt = await tribeone.debtBalanceOf(account1, toBytes32('uUSD'));
			...
			describe('adding and removing synths', () => {
				...
				describe('when another synth is added with 0 supply', () => {
					...
					it('should be able to query multiple synth addresses', async () => {
						const synthAddresses = await issuer.getSynths([currencyKey, sETH, uUSD]);
						assert.equal(synthAddresses[0], synth.address);
						assert.equal(synthAddresses[1], sETHContract.address);
						assert.equal(synthAddresses[2], uUSDContract.address);
					...
					describe('when that synth has issued but has no rate', () => {
						...
						describe('when the synth has a rate', () => {
							...
							describe('when another user exchanges into the synth', () => {
								beforeEach(async () => {
									await uUSDContract.issue(account2, toUnit('1000'));
									await tribeone.exchange(uUSD, toUnit('100'), currencyKey, { from: account2 });
								});
								...
								describe('when the same user exchanges out of the synth', () => {
									beforeEach(async () => {
										...
										await tribeone.exchange(currencyKey, toUnit('1'), uUSD, { from: account2 });
									});
									describe('when the synth is removed', () => {
										beforeEach(async () => {
											await issuer.removeSynth(currencyKey, { from: owner });
										...
										it('then settling works as expected', async () => {
											await tribeone.settle(uUSD);

											const { numEntries } = await exchanger.settlementOwing(owner, uUSD);
											assert.equal(numEntries, '0');
										});
							...
							describe('when a debt snapshot is taken', () => {
								...
								beforeEach(async () => {
									...
									totalIssuedSynths = await issuer.totalIssuedSynths(uUSD, true);
								...
								describe('when the synth is removed', () => {
									...
									beforeEach(async () => {
										// base conditions
										assert.equal(await uUSDContract.balanceOf(synthRedeemer.address), '0');
									...
									it('issues the equivalent amount of uUSD', async () => {
										const amountOfuUSDIssued = await uUSDContract.balanceOf(synthRedeemer.address);

										// 100 units of sBTC at a rate of 2:1
										assert.bnEqual(amountOfuUSDIssued, toUnit('200'));
									});
									...
									it('and total debt remains unchanged', async () => {
										assert.bnEqual(await issuer.totalIssuedSynths(uUSD, true), totalIssuedSynths);
									});
								});
							});
						});
				...
				describe('multiple add/remove synths', () => {
					...
					it('should disallow removing uUSD', async () => {
						// Assert that we can't remove uUSD
						await assert.revert(
							issuer.removeSynths([currencyKey, uUSD], { from: owner }),
							'Cannot remove synth'
						);
					});
			...
			describe('issuance', () => {
				...
				it('should allow an issuer to issue synths in one flavour', async () => {
					...
					// There should be 10 uUSD of value in the system
					assert.bnEqual(await tribeone.totalIssuedSynths(uUSD), toUnit('10'));

					// And account1 should own 100% of the debt.
					assert.bnEqual(await tribeone.totalIssuedSynths(uUSD), toUnit('10'));
					assert.bnEqual(await tribeone.debtBalanceOf(account1, uUSD), toUnit('10'));
				});
				...
				it('should allow two issuers to issue synths in one flavour', async () => {
					...
					// There should be 30uUSD of value in the system
					assert.bnEqual(await tribeone.totalIssuedSynths(uUSD), toUnit('30'));
					...
					assert.bnClose(await tribeone.debtBalanceOf(account1, uUSD), toUnit('10'));
					assert.bnClose(await tribeone.debtBalanceOf(account2, uUSD), toUnit('20'));
				});
				...
				it('should allow multi-issuance in one flavour', async () => {
					...
					// There should be 40 uUSD of value in the system
					assert.bnEqual(await tribeone.totalIssuedSynths(uUSD), toUnit('40'));

					// And the debt should be split 50/50.
					// But there's a small rounding error.
					// This is ok, as when the last person exits the system, their debt percentage is always 100% so
					// these rounding errors don't cause the system to be out of balance.
					assert.bnClose(await tribeone.debtBalanceOf(account1, uUSD), toUnit('20'));
					assert.bnClose(await tribeone.debtBalanceOf(account2, uUSD), toUnit('20'));
				});
				...
				describe('issueMaxSynths', () => {
					it('should allow an issuer to issue max synths in one flavour', async () => {
						...
						// There should be 200 uUSD of value in the system
						assert.bnEqual(await tribeone.totalIssuedSynths(uUSD), toUnit('200'));

						// And account1 should own all of it.
						assert.bnEqual(await tribeone.debtBalanceOf(account1, uUSD), toUnit('200'));
					});
				});
				...
				it('should disallow an issuer from issuing synths beyond their remainingIssuableSynths', async () => {
					...
					// They should now be able to issue uUSD
				...
				it('circuit breaks when debt changes dramatically', async () => {
					...
					assert.bnEqual(await uUSDContract.balanceOf(account1), toUnit('101'));

					await uUSDContract.issue(account1, toUnit('10000000'));
					await debtCache.takeDebtSnapshot();

					assert.bnEqual(await uUSDContract.balanceOf(account1), toUnit('10000101'));
					...
					// undo
					await uUSDContract.burn(account1, toUnit('10000000'));
			...
			describe('burning', () => {
				it('circuit breaks when debt changes dramatically', async () => {
					...
					await uUSDContract.burn(account1, toUnit('90'));
					await debtCache.takeDebtSnapshot();

					// all debt should be burned here
					assert.bnEqual(await uUSDContract.balanceOf(account1), toUnit(9));
					...
					// mint it back
					await uUSDContract.issue(account1, toUnit('90'));

					await tribeone.burnSynths('1', { from: account1 });
					await tribeone.burnSynths('1', { from: account1 });

					// debt should not have changed
					assert.bnEqual(await uUSDContract.balanceOf(account1), toUnit(99));
				});
				...
				it('should allow an issuer with outstanding debt to burn synths and decrease debt', async () => {
					...
					// account1 should now have 200 uUSD of debt.
					assert.bnEqual(await tribeone.debtBalanceOf(account1, uUSD), toUnit('200'));

					// Burn 100 uUSD
					await tribeone.burnSynths(toUnit('100'), { from: account1 });

					// account1 should now have 100 uUSD of debt.
					assert.bnEqual(await tribeone.debtBalanceOf(account1, uUSD), toUnit('100'));
				});
				...
				it('should disallow an issuer without outstanding debt from burning synths', async () => {
					...
					await uUSDContract.transfer(account2, toUnit('100'), {
						from: account1,
					});
				...
				it('should revert when trying to burn synths that do not exist', async () => {
					...
					await uUSDContract.transfer(account2, toUnit('200'), {
						from: account1,
					});

					const debtBefore = await tribeone.debtBalanceOf(account1, uUSD);

					assert.ok(!debtBefore.isNeg());

					// Burning any amount of uUSD beyond what is owned will cause a revert
					await assert.revert(
				...
				it("should only burn up to a user's actual debt level", async () => {
					...
					await uUSDContract.transfer(account1, amountTransferred, {
						from: account2,
					});
					// return;

					const balanceOfAccount1 = await uUSDContract.balanceOf(account1);
					...
					const balanceOfAccount1AfterBurn = await uUSDContract.balanceOf(account1);
				...
				it("should successfully burn all user's synths @gasprofile", async () => {
					...
					// Then try to burn them all. Only 10 synths (and fees) should be gone.
					await tribeone.burnSynths(await uUSDContract.balanceOf(account1), {
						from: account1,
					});

					assert.bnEqual(await uUSDContract.balanceOf(account1), web3.utils.toBN(0));
				});
				...
				it('should burn the correct amount of synths', async () => {
					...
					const debt = await tribeone.debtBalanceOf(account1, uUSD);
				...
				describe('debt calculation in multi-issuance scenarios', () => {
					it('should correctly calculate debt in a multi-issuance multi-burn scenario @gasprofile', async () => {
						...
						const debtBalance1After = await tribeone.debtBalanceOf(account1, uUSD);
						const debtBalance2After = await tribeone.debtBalanceOf(account2, uUSD);
						const debtBalance3After = await tribeone.debtBalanceOf(account3, uUSD);
					...
					it('should allow user to burn all synths issued even after other users have issued', async () => {
						...
						const debtBalanceBefore = await tribeone.debtBalanceOf(account1, uUSD);
						await tribeone.burnSynths(debtBalanceBefore, { from: account1 });
						const debtBalanceAfter = await tribeone.debtBalanceOf(account1, uUSD);
					...
					it('should allow users to burn their debt and adjust the debtBalanceOf correctly for remaining users', async () => {
						...
						let debtBalance1After = await tribeone.debtBalanceOf(account1, uUSD);
						let debtBalance2After = await tribeone.debtBalanceOf(account2, uUSD);
						...
						debtBalance1After = await tribeone.debtBalanceOf(account1, uUSD);
						debtBalance2After = await tribeone.debtBalanceOf(account2, uUSD);
				...
				describe('burnSynthsToTarget', () => {
					beforeEach(async () => {
						...
						// Issue
						await tribeone.issueMaxSynths({ from: account1 });
						assert.bnClose(await tribeone.debtBalanceOf(account1, uUSD), toUnit('8000'));
					...
					describe('when the HAKA price drops 50%', () => {
						...
						it('then calling burnSynthsToTarget() reduces uUSD to c-ratio target', async () => {
							await tribeone.burnSynthsToTarget({ from: account1 });
							assert.bnClose(await tribeone.debtBalanceOf(account1, uUSD), toUnit('4000'));
						});
					...
					describe('when the HAKA price drops 10%', () => {
						...
						it('then calling burnSynthsToTarget() reduces uUSD to c-ratio target', async () => {
							await tribeone.burnSynthsToTarget({ from: account1 });
							assert.bnEqual(await tribeone.debtBalanceOf(account1, uUSD), toUnit('7200'));
						});
						it('then fees are claimable', async () => {
							await tribeone.burnSynthsToTarget({ from: account1 });
							assert.equal(await feePool.isFeesClaimable(account1), true);
					...
					describe('when the HAKA price drops 90%', () => {
						...
						it('then calling burnSynthsToTarget() reduces uUSD to c-ratio target', async () => {
							await tribeone.burnSynthsToTarget({ from: account1 });
							assert.bnEqual(await tribeone.debtBalanceOf(account1, uUSD), toUnit('800'));
						});
						it('then fees are claimable', async () => {
							await tribeone.burnSynthsToTarget({ from: account1 });
							assert.equal(await feePool.isFeesClaimable(account1), true);
				...
				describe('burnSynths() after exchange()', () => {
					describe('given the waiting period is set to 60s', () => {
						...
						describe('and a user has 1250 uUSD issued', () => {
							beforeEach(async () => {
							...
							describe('and is has been exchanged into sEUR at a rate of 1.25:1 and the waiting period has expired', () => {
								beforeEach(async () => {
									await tribeone.exchange(uUSD, amount, sEUR, { from: account1 });
									await fastForward(90); // make sure the waiting period is expired on this
								});
								describe('and they have exchanged all of it back into uUSD', () => {
									beforeEach(async () => {
										await tribeone.exchange(sEUR, toUnit('1000'), uUSD, { from: account1 });
									});
									describe('when they attempt to burn the uUSD', () => {
									...
									describe('and 60s elapses with no change in the sEUR rate', () => {
										...
										describe('when they attempt to burn the uUSD', () => {
											...
											it('then it succeeds and burns the entire uUSD amount', async () => {
												const logs = await getDecodedLogs({
													hash: txn.tx,
													contracts: [tribeone, uUSDContract],
												});

												decodedEventEqual({
													event: 'Burned',
													emittedFrom: uUSDContract.address,
													args: [account1, amount],
													log: logs.find(({ name } = {}) => name === 'Burned'),
												});

												const uUSDBalance = await uUSDContract.balanceOf(account1);
												assert.equal(uUSDBalance, '0');

												const debtBalance = await tribeone.debtBalanceOf(account1, uUSD);
									...
									describe('and the sEUR price decreases by 20% to 1', () => {
										...
										describe('and 60s elapses', () => {
											...
											describe('when they attempt to burn the entire amount uUSD', () => {
												let txn;
												beforeEach(async () => {
													txn = await tribeone.burnSynths(amount, { from: account1 });
												});
												it('then it succeeds and burns their uUSD minus the reclaim amount from settlement', async () => {
													const logs = await getDecodedLogs({
														hash: txn.tx,
														contracts: [tribeone, uUSDContract],
													});
												...
												it('and their debt balance is now 0 because they are the only debt holder in the system', async () => {
													...
													const debtBalance = await tribeone.debtBalanceOf(account1, uUSD);
													// because this user is the only one holding debt, when we burn 250 uUSD in a reclaim,
											...
											describe('when another user also has the same amount of debt', () => {
												...
												describe('when the first user attempts to burn the entire amount uUSD', () => {
													let txn;
													beforeEach(async () => {
														txn = await tribeone.burnSynths(amount, { from: account1 });
													});
													...
													it('and their debt balance is now half of the reclaimed balance because they owe half of the pool', async () => {
														// the debt balance remaining is what was reclaimed from the exchange
														const debtBalance = await tribeone.debtBalanceOf(account1, uUSD);
														// because this user is holding half the debt, when we burn 250 uUSD in a reclaim,
			...
			describe('debt calculation in multi-issuance scenarios', () => {
				...
				it("should allow me to burn all synths I've issued when there are other issuers", async () => {
					...
					await tribeone.issueMaxSynths({ from: account1 });
					const debtBalance1 = await tribeone.debtBalanceOf(account1, uUSD);
					...
					let debt = await tribeone.debtBalanceOf(account2, uUSD);

					// due to rounding it may be necessary to supply higher than originally issued synths
					await uUSDContract.transfer(account2, toUnit('1'), {
						from: account1,
					});
					await tribeone.burnSynths(toUnit('44'), { from: account2 });
					debt = await tribeone.debtBalanceOf(account2, uUSD);
			...
			describe('multiple issue and burn scenarios', () => {
				it('should correctly calculate debt in a high issuance and burn scenario', async () => {
					...
					for (let i = 0; i < totalTimesToIssue; i++) {
						...
						// const db = await tribeone.debtBalanceOf(account2, uUSD);
					...
					const debtBalance = await tribeone.debtBalanceOf(account2, uUSD);
				...
				it('should correctly calculate debt in a high volume contrast issuance and burn scenario', async () => {
					...
					const debtBalance2 = await tribeone.debtBalanceOf(account2, uUSD);
			...
			it("should prevent more issuance if the user's collaterisation changes to be insufficient", async () => {
				...
				// exchange into sEUR
				await tribeone.exchange(uUSD, issuedSynths, sEUR, { from: account1 });
			...
			describe('check collaterisation ratio', () => {
				...
				it('should permit user to issue uUSD debt with only escrowed HAKA as collateral (no HAKA in wallet)', async () => {
					...
					// Issue max synths. (300 uUSD)
					await tribeone.issueMaxSynths({ from: account1 });

					// There should be 300 uUSD of value for account1
					assert.bnEqual(await tribeone.debtBalanceOf(account1, uUSD), toUnit('300'));
				});

				it('should permit user to issue uUSD debt with only reward escrow as collateral (no HAKA in wallet)', async () => {
					// ensure collateral of account1 is empty
					let collateral = await tribeone.collateral(account1, { from: account1 });
					assert.bnEqual(collateral, 0);
			...
			describe('issue and burn on behalf', async () => {
				...
				it('should approveIssueOnBehalf and IssueMaxSynths', async () => {
					...
					const uUSDBalanceBefore = await uUSDContract.balanceOf(account1);
					const issuableSynths = await tribeone.maxIssuableSynths(account1);

					await tribeone.issueMaxSynthsOnBehalf(authoriser, { from: delegate });
					const uUSDBalanceAfter = await uUSDContract.balanceOf(account1);
					assert.bnEqual(uUSDBalanceAfter, uUSDBalanceBefore.add(issuableSynths));
				});
				it('should approveIssueOnBehalf and IssueSynths', async () => {
					...
					const uUSDBalance = await uUSDContract.balanceOf(account1);
					assert.bnEqual(uUSDBalance, toUnit('100'));
				});
				it('should approveBurnOnBehalf and BurnSynths', async () => {
					...
					const uUSDBalanceBefore = await uUSDContract.balanceOf(account1);
					await tribeone.burnSynthsOnBehalf(authoriser, uUSDBalanceBefore, { from: delegate });

					const uUSDBalance = await uUSDContract.balanceOf(account1);
					assert.bnEqual(uUSDBalance, toUnit('0'));
				});
				it('should approveBurnOnBehalf and burnSynthsToTarget', async () => {
					...
					const uUSDBalanceAfter = await uUSDContract.balanceOf(account1);
					assert.bnEqual(uUSDBalanceAfter, toUnit('40'));
				});
			});
			...
			describe('when Wrapper is set', async () => {
				it('should have zero totalIssuedSynths', async () => {
					assert.bnEqual(
						await tribeone.totalIssuedSynths(uUSD),
						await tribeone.totalIssuedSynthsExcludeOtherCollateral(uUSD)
					);
				});
			...
			describe('debt shares integration', async () => {
				...
				it('burns the correct number of debt shares', async () => {
					...
					assert.bnEqual(await tribeone.debtBalanceOf(account1, uUSD), toUnit('270'));
				});
				...
				describe('when debt ratio changes', () => {
					...
					it('has adjusted debt', async () => {
						assert.bnEqual(await tribeone.debtBalanceOf(account1, uUSD), toUnit('450')); // = 750 sds * 0.6
					});
					...
					it('mints at adjusted rate', async () => {
						...
						assert.bnEqual(await tribeone.debtBalanceOf(account1, uUSD), toUnit('750')); // = 450 (uUSD from before ) + 300
					});
				});
				...
				describe('issued synths aggregator', async () => {
					...
					it('has no effect on mint or burn', async () => {
						...
						assert.bnEqual(await tribeone.debtBalanceOf(account1, uUSD), toUnit('270')); // 300 - 30 susd
					});
				});
			});
			...
			describe('upgradeCollateralShort', () => {
				...
				describe('migrates balance', () => {
					...
					beforeEach(async () => {
						...
						// get before* values
						beforeSUSDBalance = await uUSDContract.balanceOf(collateralShortMock);
					...
					it('burns synths', async () => {
						assert.bnEqual(
							await uUSDContract.balanceOf(collateralShortMock),
