...
contract('Rewards Integration Tests', accounts => {
	...
	// CURRENCIES
	const [uUSD, sAUD, sEUR, sBTC, HAKA, iBTC, sETH, ETH] = [
		'uUSD',
		'sAUD',
		'sEUR',
		'sBTC',
	...
	const synthKeys = [uUSD, sAUD, sEUR, sBTC, iBTC, sETH, ETH];
	...
	before(async function() {
		...
		({
			...
			accounts,
			synths: ['uUSD', 'sAUD', 'sEUR', 'sBTC', 'iBTC', 'sETH'],
			contracts: [
				'AddressResolver',
	...
	describe('3 accounts with 33.33% HAKA all issue MAX and claim rewards', async () => {
		...
		it('should allow a user to leave the system and return and still claim rewards', async () => {
			...
			const burnableTotal = await tribeone.debtBalanceOf(account1, uUSD);
	...
	describe('Exchange Rate Shift tests', async () => {
		it('should assign accounts (1,2,3) to have (40%,40%,20%) of the debt/rewards', async () => {
			...
			await tribeone.exchange(uUSD, tenK, sBTC, { from: account1 });
			await tribeone.exchange(uUSD, tenK, sBTC, { from: account2 });
			...
			// Account 3 (enters the system and) mints 10K uUSD (minus half of an exchange fee - to balance the fact
			...
			const { amountReceived } = await exchanger.getAmountsForExchange(tenK, uUSD, sBTC);
			await tribeone.exchange(sBTC, amountReceived, uUSD, { from: account1 });
			await tribeone.exchange(sBTC, amountReceived, uUSD, { from: account2 });
			...
			// // Account 1 exchanges all sBTC back to uUSD
			// const acc1sBTCBalance = await sBTCContract.balanceOf(account1, { from: account1 });
			// await tribeone.exchange(sBTC, acc1sBTCBalance, uUSD, { from: account1 });
	...
	describe('3 Accounts issue 10K uUSD each in week 1', async () => {
		beforeEach(async () => {
			await tribeone.issueSynths(tenK, { from: account1 });
			await tribeone.issueSynths(tenK, { from: account2 });
		...
		it('Acc1 issues and burns multiple times and should have accounts 1,2,3 rewards 50%,25%,25%', async () => {
			...
			// Acc 1 Issues 10K uUSD
			await tribeone.issueSynths(tenK, { from: account1 });
			// Acc 1 Issues 10K uUSD again
			await tribeone.issueSynths(tenK, { from: account1 });
	...
	describe('When user is the last to call claimFees()', () => {
		beforeEach(async () => {
			...
			await tribeone.exchange(uUSD, oneThousand, sAUD, { from: account2 });
			await tribeone.exchange(uUSD, oneThousand, sAUD, { from: account1 });
		...
		it('then account gets remainder of fees/rewards available after wei rounding', async () => {
			...
			const olduUSDBalance = await uUSDContract.balanceOf(account2);
			...
			assert.eventEqual(claimFeesTx, 'FeesClaimed', {
				uUSDAmount: feesAvailableUSD[0],
				snxRewards: feesAvailableUSD[1],
			});

			const newUSDBalance = await uUSDContract.balanceOf(account2);
			// uUSD balance remains unchanged since the fees are burned.
			assert.bnEqual(newUSDBalance, olduUSDBalance);

			const period = await feePool.recentFeePeriods(1);
			period.index = 1;

			// Simulate rounding on uUSD leaving fraction less for the last claimer.
			...
			assert.eventEqual(transaction, 'FeesClaimed', {
				uUSDAmount: feesAvailableUSDAcc1[0],
