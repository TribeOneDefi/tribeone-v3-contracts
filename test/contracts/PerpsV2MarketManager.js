...
contract('FuturesMarketManager (PerpsV2)', accounts => {
	...
	before(async () => {
		...
		await uUSD.issue(trader, initialMint, { from: owner });
	});

	addSnapshotBeforeRestoreAfterEach();

	describe('Basic parameters', () => {
		it('Requires uUSD contract', async () => {
			const required = await futuresMarketManager.resolverAddressesRequired();
			assert.deepEqual(required, ['SynthuUSD', 'FeePool', 'Exchanger'].map(toBytes32));
		});
	...
	describe('Market management', () => {
		...
		describe('Update market implememtations', () => {
			...
			it('can update a market', async () => {
				...
				assert.bnEqual(await uUSD.balanceOf(owner), toUnit('10'));
			});
		});
	});

	describe('uUSD issuance', () => {
		let market, proxy;
		beforeEach(async () => {
		...
		it('issuing/burning uUSD', async () => {
			await market.issueSUSD(owner, toUnit('10'));
			assert.bnEqual(await uUSD.balanceOf(owner), toUnit('10'));

			await market.burnSUSD(owner, toUnit('5'));
			assert.bnEqual(await uUSD.balanceOf(owner), toUnit('5'));
		...
		it('burning respects settlement', async () => {
			...
			assert.bnEqual(await uUSD.balanceOf(owner), toUnit('100'));
		...
		it('only markets are permitted to issue or burn uUSD', async () => {
			await onlyGivenAddressCanInvoke({
	...
	describe('Pay fees', () => {
		...
		it('pays fees', async () => {
			...
			assert.bnEqual(await uUSD.balanceOf(FEE_ADDRESS), preBalance.add(toUnit('10')));
		});
	...
	describe('Aggregated Debt', () => {
		it('perpsV2 debt is zero when no markets are deployed', async () => {
			...
			// issue some uUSD
			uUSD.issue(trader, toUnit(100), { from: owner });
	...
	describe('Market summaries', () => {
		...
		beforeEach(async () => {
			...
			// Issue the traders some uUSD
			await uUSD.issue(trader, traderInitialBalance);
