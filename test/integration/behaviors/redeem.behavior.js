...
function itCanRedeem({ ctx }) {
	describe('redemption of deprecated synths', () => {
		let owner;
		let someUser;
		let TribeOne, Issuer, SynthToRedeem, SynthuUSD, SynthToRedeemProxy, SynthRedeemer;
		...
		before('ensure the user has uUSD', async () => {
			await ensureBalance({
				ctx,
				symbol: 'uUSD',
				user: someUser,
		...
		before('record total system debt', async () => {
			totalDebtBeforeRemoval = await Issuer.totalIssuedSynths(toBytes32('uUSD'), true);
		});
		...
		describe(`deprecating the synth`, () => {
			...
			it('then the total system debt is unchanged', async () => {
				assert.bnEqual(
					await Issuer.totalIssuedSynths(toBytes32('uUSD'), true),
			...
			describe('user redemption', () => {
				let uUSDBeforeRedemption;
				before(async () => {
					uUSDBeforeRedemption = await SynthuUSD.balanceOf(someUser.address);
				});
				...
				it('and they have more uUSD again', async () => {
					assert.bnGt(await SynthuUSD.balanceOf(someUser.address), uUSDBeforeRedemption);
				});
			});
		});
	});
