...
function itCanOpenAndCloseShort({ ctx }) {
	describe('shorting', () => {
		const amountOfuUSDRequired = parseEther('5000'); // uUSD
		const amountToDeposit = parseEther('1000'); // uUSD
		const amountToBorrow = parseEther('0.00000000001'); // sETH
		const amountToExchange = parseEther('100'); // uUSD

		const shortableSynth = toBytes32('sETH');

		let user, owner;
		let CollateralShort, CollateralManager, TribeOne, SynthuUSD, interactionDelay;

		before('target contracts and users', () => {
			({ CollateralShort, CollateralManager, TribeOne, SynthuUSD } = ctx.contracts);
		...
		describe('when opening is enabled', () => {
			before('ensure user should have uUSD', async () => {
				await ensureBalance({ ctx, symbol: 'uUSD', user, balance: amountOfuUSDRequired });
			});
				...
				describe('open a loan, deposit and withdraw collateral, draw, and close the loan', () => {
					...
					describe('closing a loan', () => {
						...
						before('skip waiting period', async () => {
							// Ignore settlement period for uUSD --> sETH closing the loan
