...
function itCanTrade({ ctx }) {
	describe('opening positions', function() {
		...
		const uUSDAmount = ethers.utils.parseEther('100000');
		...
		before('ensure users have uUSD ', async () => {
			await ensureBalance({ ctx, symbol: 'uUSD', user: someUser, balance: uUSDAmount });
		});

		after('reset the uUSD balance', async () => {
			await ensureBalance({ ctx, symbol: 'uUSD', user: someUser, balance: toBN(0) });
		});
		...
		describe('position management', () => {
			...
			it('user can transferMargin and withdraw it', async () => {
				...
				const balance = await SynthuUSD.balanceOf(someUser.address);
				// transfer
				await market.transferMargin(margin);
				assert.bnEqual(await SynthuUSD.balanceOf(someUser.address), balance.sub(margin));

				// withdraw
				await (await market.withdrawAllMargin()).wait();
				const withdrawBalance = await SynthuUSD.balanceOf(someUser.address);
