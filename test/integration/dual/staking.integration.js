...
describe('staking & claiming integration tests (L1, L2)', () => {
	...
	describe('staking and claiming', () => {
		...
		let user;
		let TribeOne, SynthuUSD, FeePool;
		let balanceuUSD, debtuUSD;

		before('target contracts and users', () => {
			({ TribeOne, SynthuUSD, FeePool } = ctx.l1.contracts);
			...
			describe('claiming', () => {
				...
				describe('when the fee period closes', () => {
					...
					describe('when the user claims rewards', () => {
						...
						it('shows no change in the users uUSD balance', async () => {
							assert.bnEqual(await SynthuUSD.balanceOf(user.address), balanceuUSD);
						});
					});
				});
			});

			describe('when the user burns uUSD', () => {
