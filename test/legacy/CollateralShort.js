...
contract('CollateralShort', async accounts => {
	...
	const setupShort = async () => {
		...
		await manager.addSynths(
			[toBytes32('SynthuUSD'), toBytes32('SynthsBTC'), toBytes32('SynthsETH')],
			[toBytes32('uUSD'), toBytes32('sBTC'), toBytes32('sETH')],
			{
				from: owner,
			}
		);
		...
		assert.isTrue(
			await manager.areSynthsAndCurrenciesSet(
				['SynthuUSD', 'SynthsBTC', 'SynthsETH'].map(toBytes32),
				['uUSD', 'sBTC', 'sETH'].map(toBytes32)
			)
		);
		...
		assert.isTrue(await manager.isSynthManaged(uUSD));
		...
		await uUSDSynth.approve(short.address, toUnit(100000), { from: account1 });
	};
	...
	before(async () => {
		...
		await issue(uUSDSynth, toUnit(100000), owner);
	...
	describe('logic', () => {
		...
		it('should set constructor params on deployment', async () => {
			...
			assert.equal(await short.collateralKey(), uUSD);
		...
		it('should access its dependencies via the address resolver', async () => {
			assert.equal(await addressResolver.getAddress(toBytes32('SynthuUSD')), uUSDSynth.address);
		...
		describe('opening shorts', async () => {
			describe('should open a btc short', async () => {
				...
				beforeEach(async () => {
					await issue(uUSDSynth, susdCollateral, account1);
				...
				it('should correclty issue the right balance to the shorter', async () => {
					const uUSDProceeds = toUnit(10000);

					assert.bnEqual(await uUSDSynth.balanceOf(account1), uUSDProceeds);
				});
				...
				it('should transfer the uUSD to the contract', async () => {
					assert.bnEqual(await uUSDSynth.balanceOf(short.address), susdCollateral);
				});
			});
		...
		describe('Repaying shorts', async () => {
			...
			beforeEach(async () => {
				...
				beforeFeePoolBalance = await uUSDSynth.balanceOf(FEE_ADDRESS);
				beforeShortBalance = await uUSDSynth.balanceOf(short.address);
				beforeUserBalance = await uUSDSynth.balanceOf(account1);
			...
			it('should repay with collateral and update the loan', async () => {
				...
				const {
					...
					fee: exchangeFee,
				} = await exchanger.getAmountsForExchange(ethAmountToRepay, sETH, uUSD);
				...
				assert.deepEqual(
					await uUSDSynth.balanceOf(FEE_ADDRESS),
				...
				// The contract uUSD balance should have been reduced by the expected amount
				assert.deepEqual(
					await uUSDSynth.balanceOf(short.address),
					beforeShortBalance.sub(collateralToUse),
					'The short contracts holds excess uUSD'
				);

				// The user uUSD balance should remain unchanged
				assert.deepEqual(
					await uUSDSynth.balanceOf(account1),
					beforeUserBalance,
					'The user uUSD balance is unexpected'
				);
			});
			...
			it('should repay with collateral and update the loan considering interest accrued', async () => {
				...
				const uUSDAccruedInterest = await exchangeRates.effectiveValue(
					loan.currency,
					accruedInterest,
					uUSD
				);
				...
				const {
					amountReceived: susdAmountRepaidMinusFees,
					fee: exchangeFee,
				} = await exchanger.getAmountsForExchange(ethAmountToRepay, sETH, uUSD); // ethAmountToRepay
				...
				assert.deepEqual(
					await uUSDSynth.balanceOf(FEE_ADDRESS),
					beforeFeePoolBalance.add(uUSDAccruedInterest).add(exchangeFee),
					'The fee pool did not receive enough fees'
				);
		...
		describe('Closing shorts', () => {
			...
			it('should repay with collateral and close the loan', async () => {
				...
				assert.bnEqual(await uUSDSynth.balanceOf(account1), toUnit(100));
				...
				// The user uUSD balance should increase
				assert.deepEqual(
					await uUSDSynth.balanceOf(account1),
			...
			it('should repay with collateral and close the loan considering interest accrued', async () => {
				...
				assert.bnEqual(await uUSDSynth.balanceOf(account1), beforeUserBalance);
				...
				assert.deepEqual(
					await uUSDSynth.balanceOf(FEE_ADDRESS),
					beforeFeePoolBalance.add(exchangeFee).add(uUSDAccruedInterest),
					'The fee pool did not receive enough fees'
				);
				...
				// The user uUSD balance should reduce by the fees paid
				assert.deepEqual(
					await uUSDSynth.balanceOf(account1),
					beforeUserShortBalance.sub(exchangeFee).sub(uUSDAccruedInterest),
					'The user uUSD balance is unexpected'
				);
			});
		});
		...
		describe('Drawing shorts', async () => {
			...
			it('should transfer the proceeds to the user', async () => {
				assert.bnEqual(await uUSDSynth.balanceOf(account1), toUnit(600));
			});
		...
		describe('Withdrawing shorts', async () => {
			...
			beforeEach(async () => {
				...
				previousBalance = await uUSDSynth.balanceOf(account1);
			...
			it('should transfer the withdrawn collateral to the user', async () => {
				assert.bnEqual(await uUSDSynth.balanceOf(account1), toUnit(100).add(previousBalance));
			});
		...
		describe('Closing shorts', async () => {
			...
			it('if the eth price goes down, the shorter makes profit', async () => {
				...
				await uUSDSynth.transfer(owner, toUnit(50), { from: account1 });
				await issue(sETHSynth, oneETH, account1);

				// now close the short
				await short.close(id, { from: account1 });

				// shorter has made 50 uUSD profit
				assert.bnEqual(await uUSDSynth.balanceOf(account1), toUnit(1050));
			});
			...
			it('if the eth price goes up, the shorter makes a loss', async () => {
				...
				await uUSDSynth.transfer(owner, toUnit(150), { from: account1 });
				await issue(sETHSynth, oneETH, account1);

				// now close the short
				await short.close(id, { from: account1 });

				// shorter has made 50 uUSD loss
				assert.bnEqual(await uUSDSynth.balanceOf(account1), toUnit(950));
			});
		});
		...
		describe('Liquidating shorts', async () => {
			...
			const getExpectedValues = async ({
				...
				const liquidatedAmountuUSD = divideDecimal(dividend, divisor);

				const liquidatedLoan = await exchangeRates.effectiveValue(uUSD, liquidatedAmountuUSD, sETH);
				const remainingLoan = initialLoan.sub(liquidatedLoan);
				const liquidatedCollateral = multiplyDecimal(
					await exchangeRates.effectiveValue(sETH, liquidatedLoan, uUSD),
					one.add(penalty)
				);
		...
		describe('System debt', async () => {
			...
			it('If there is 1 ETH and 1 short ETH, then the system debt is constant before and after a price change', async () => {
				...
				// simulate buying sETH for 150 susd.
				await uUSDSynth.burn(account1, toUnit(150));
				await issue(sETHSynth, oneETH, account1);
			...
			it('If there is 1 ETH and 2 short ETH, then the system debt decreases if the price goes up', async () => {
				...
				await uUSDSynth.burn(account1, toUnit(300));
				...
				// shorter has made 50 uUSD loss
				assert.bnEqual(await uUSDSynth.balanceOf(account1), toUnit(900));
			});
			...
			it('If there is 1 ETH and 2 short ETH, then the system debt increases if the price goes down', async () => {
				...
				await uUSDSynth.burn(account1, toUnit(100));
				await issue(sETHSynth, twoETH, account1);
				...
				// shorter has made 100 uUSD profit
				assert.bnEqual(await uUSDSynth.balanceOf(account1), toUnit(1100));
			});
		});
