...
contract('CollateralErc20', async accounts => {
	...
	it('should set constructor params on deployment', async () => {
		...
		assert.equal(await cerc20.synths(0), toBytes32('SynthuUSD'));
	...
	describe('max loan test', async () => {
		it('should convert correctly', async () => {
			// $150 worth of btc should allow 100 uUSD to be issued.
			const uUSDAmount = await cerc20.maxLoan(toUnit(0.015), uUSD);

			assert.bnClose(uUSDAmount, toUnit(100), 100);
	...
	describe('opening', async () => {
		describe('potential blocking conditions', async () => {
			['System', 'Issuance'].forEach(section => {
				describe(`when ${section} is suspended`, () => {
					...
					it('then calling openLoan() reverts', async () => {
						await assert.revert(
							cerc20.open(oneRenBTC, oneuUSD, uUSD, { from: account1 }),
					...
					describe(`when ${section} is resumed`, () => {
						...
						it('then calling openLoan() succeeds', async () => {
							await cerc20.open(oneRenBTC, oneuUSD, uUSD, {
								from: account1,
							});
						});
					});
			...
			describe('when rates have gone stale', () => {
				...
				describe('when BTC gets a rate', () => {
					...
					it('then calling openLoan() succeeds', async () => {
						await cerc20.open(oneRenBTC, oneuUSD, uUSD, { from: account1 });
					});
				});
			});
		});
		...
		describe('revert conditions', async () => {
			it('should revert if they request a currency that is not supported', async () => {
				await assert.revert(
					cerc20.open(oneRenBTC, oneuUSD, toBytes32('sJPY'), { from: account1 }),
			...
			it('should revert if they send 0 collateral', async () => {
				await assert.revert(
					cerc20.open(toUnit(0), oneuUSD, uUSD, { from: account1 }),
			...
			it('should revert if the requested loan exceeds borrowing power', async () => {
				await assert.revert(
					cerc20.open(oneRenBTC, toUnit(10000), uUSD, {
						from: account1,
					}),
		...
		describe('should open a btc loan denominated in uUSD', async () => {
			const fiveHundredSUSD = toUnit(500);
			let issueFeeRate;
			let issueFee;

			beforeEach(async () => {
				tx = await cerc20.open(oneRenBTC, fiveHundredSUSD, uUSD, {
					from: account1,
				});
	...
	describe('deposits', async () => {
		beforeEach(async () => {
			tx = await cerc20.open(twoRenBTC, oneHundreduUSD, uUSD, {
				from: account1,
			});
	...
	describe('withdraws', async () => {
		...
		beforeEach(async () => {
			loan = await cerc20.open(twoRenBTC, oneHundreduUSD, uUSD, {
				from: account1,
			});
	...
	describe('repayments', async () => {
			...
			['System', 'Issuance'].forEach(section => {
				describe(`when ${section} is suspended`, () => {
					...
					it('then calling repay() reverts', async () => {
						await assert.revert(
							cerc20.repay(account1, id, oneuUSD, { from: account1 }),
							'Operation prohibited'
						);
					});
					describe(`when ${section} is resumed`, () => {
						...
						it('then calling repay() succeeds', async () => {
							await cerc20.repay(account1, id, oneuUSD, { from: account1 });
						});
					});
				});
			});
		...
		describe('revert conditions', async () => {
			...
			it('should revert if they have no uUSD', async () => {
				await assert.revert(
					cerc20.repay(account1, id, tenuUSD, { from: account2 }),
					'Not enough balance'
				);
			});
		...
		describe('should allow repayments on an uUSD loan', async () => {
			...
			beforeEach(async () => {
				...
				tx = await cerc20.repay(account1, id, tenuUSD, { from: account2 });
				loan = await cerc20.loans(id);
			});
	...
	describe('liquidations', async () => {
		beforeEach(async () => {
			// make a loan here so we have a valid ID to pass to the blockers and reverts.
			tx = await cerc20.open(oneRenBTC, toUnit(5000), uUSD, {
				from: account1,
			});
		...
		describe('potential blocking conditions', async () => {
			['System', 'Issuance'].forEach(section => {
				describe(`when ${section} is suspended`, () => {
					...
					it('then calling repay() reverts', async () => {
						await assert.revert(
							cerc20.liquidate(account1, id, oneuUSD, { from: account1 }),
							'Operation prohibited'
						);
					});
					describe(`when ${section} is resumed`, () => {
						...
						it('then calling liquidate() succeeds', async () => {
							...
							await cerc20.liquidate(account1, id, oneuUSD, { from: account1 });
						});
					});
				});
			});
		...
		describe('revert conditions', async () => {
			it('should revert if they have no uUSD', async () => {
				await assert.revert(
					cerc20.liquidate(account1, id, oneuUSD, { from: account2 }),
					'Not enough balance'
				);
			});
		...
		describe('should allow liquidations on an undercollateralised uUSD loan', async () => {
			...
			beforeEach(async () => {
				...
				await issueuUSDToAccount(toUnit(5000), account2);
		...
		describe('when a loan needs to be completely liquidated', async () => {
			beforeEach(async () => {
				...
				await issueuUSDToAccount(toUnit(10000), account2);
	...
	describe('drawing', async () => {
		...
		describe('potential blocking conditions', async () => {
			['System', 'Issuance'].forEach(section => {
				describe(`when ${section} is suspended`, () => {
					...
					it('then calling draw() reverts', async () => {
						await assert.revert(
							cerc20.draw(id, oneuUSD, { from: account1 }),
							'Operation prohibited'
						);
					});
					describe(`when ${section} is resumed`, () => {
			...
			describe('when rates have gone stale', () => {
				...
				it('then calling draw() reverts', async () => {
					await assert.revert(cerc20.draw(id, oneuUSD, { from: account1 }), 'Invalid rate');
				});
				describe('when BTC gets a rate', () => {
					beforeEach(async () => {
						await updateRatesWithDefaults();
					...
					it('then calling draw() succeeds', async () => {
						await cerc20.draw(id, oneuUSD, { from: account1 });
					});
				});
			});
		});
		...
		describe('should draw the loan down', async () => {
			beforeEach(async () => {
				tx = await cerc20.draw(id, oneThousanduUSD, { from: account1 });
