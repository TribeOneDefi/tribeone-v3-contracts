...
contract('CollateralManager', async accounts => {
	...
	const setupManager = async () => {
		synths = ['uUSD', 'sBTC', 'sETH', 'iBTC', 'iETH'];
		({
	...
	beforeEach(async () => {
		...
		await issue(uUSDSynth, toUnit(1000), owner);
	...
	describe('adding synths', async () => {
		...
		it('should not allow duplicate synths to be added', async () => {
			await manager.addSynths([toBytes32('SynthuUSD')], [toBytes32('uUSD')], {
				from: owner,
			});
			assert.isTrue(
				await manager.areSynthsAndCurrenciesSet(
		...
		it('should revert when input array lengths dont match', async () => {
			await assert.revert(
				manager.addSynths([toBytes32('SynthuUSD'), toBytes32('SynthsBTC')], [toBytes32('uUSD')], {
					from: owner,
				}),
		...
		it('should revert when input array lengths dont match', async () => {
			await assert.revert(
				manager.removeSynths(
					[toBytes32('SynthuUSD'), toBytes32('SynthsBTC')],
					[toBytes32('uUSD')],
					{
						from: owner,
					}
	...
	describe('should only allow opening positions up to the debt limiit', async () => {
		beforeEach(async () => {
			await issue(uUSDSynth, toUnit(15000000), account1);
			await uUSDSynth.approve(short.address, toUnit(15000000), { from: account1 });
		});
	...
	describe('tracking synth balances across collaterals', async () => {
		beforeEach(async () => {
			tx = await ceth.open(toUnit(100), uUSD, { value: toUnit(2), from: account1 });
			await ceth.open(toUnit(1), sETH, { value: toUnit(2), from: account1 });
			await cerc20.open(oneRenBTC, toUnit(100), uUSD, { from: account1 });
		...
		it('should correctly get the total uUSD balance', async () => {
			assert.bnEqual(await manager.long(uUSD), toUnit(200));
		});
		...
		it('should get the total long balance in uUSD correctly', async () => {
		...
		it('should get the total short balance in uUSD correctly', async () => {
		...
		it('should get the total long and short balance in uUSD correctly', async () => {
		...
		it('should reduce the uUSD balance when a loan is closed', async () => {
			issue(uUSDSynth, toUnit(10), account1);
			await fastForwardAndUpdateRates(INTERACTION_DELAY);
			await ceth.close(id, { from: account1 });

			assert.bnEqual(await manager.long(uUSD), toUnit(100));
		});

		it('should reduce the total balance in uUSD when a loan is closed', async () => {
			issue(uUSDSynth, toUnit(10), account1);
			await fastForwardAndUpdateRates(INTERACTION_DELAY);
			await ceth.close(id, { from: account1 });
