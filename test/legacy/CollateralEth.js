...
contract('CollateralEth', async accounts => {
	...
	const oneuUSD = toUnit(1);
	const tenuUSD = toUnit(10);
	const oneHundreduUSD = toUnit(100);
	const fiveHundreduUSD = toUnit(500);
	...
	it('should set constructor params on deployment', async () => {
		...
		assert.equal(await ceth.synths(0), toBytes32('SynthuUSD'));
	...
	describe('cratio test', async () => {
		describe('uUSD loans', async () => {
			beforeEach(async () => {
				tx = await ceth.open(oneHundreduUSD, uUSD, {
					value: twoETH,
					from: account1,
				});
	...
	describe('max loan test', async () => {
		it('should convert correctly', async () => {
			// $270 worth of eth should allow 200 uUSD to be issued.
			const uUSDAmount = await ceth.maxLoan(toUnit('2.6'), uUSD);

			assert.bnClose(uUSDAmount, toUnit('200'), '100');
	...
	describe('opening', async () => {
		describe('potential blocking conditions', async () => {
			['System', 'Issuance'].forEach(section => {
				describe(`when ${section} is suspended`, () => {
					...
					it('then calling openLoan() reverts', async () => {
						await assert.revert(
							ceth.open(oneuUSD, uUSD, { value: twoETH, from: account1 }),
			...
			describe('when rates have gone stale', () => {
				...
				describe('when ETH gets a rate', () => {
					...
					it('then calling openLoan() succeeds', async () => {
						await ceth.open(oneuUSD, uUSD, { value: twoETH, from: account1 });
					});
				});
			});
		});
		...
		describe('revert conditions', async () => {
			it('should revert if they request a currency that is not supported', async () => {
				await assert.revert(
					ceth.open(oneuUSD, toBytes32('sJPY'), { value: twoETH, from: account1 }),
			...
			it('should revert if they send 0 collateral', async () => {
				await assert.revert(
					ceth.open(oneuUSD, uUSD, { value: oneETH, from: account1 }),
			...
			it('should revert if the requested loan exceeds borrowing power', async () => {
				await assert.revert(
					ceth.open(fiveHundreduUSD, uUSD, { value: twoETH, from: account1 }),
					'Exceed max borrow power'
				);
			});
		});
		describe('should open an eth loan denominated in uUSD', async () => {
			beforeEach(async () => {
				tx = await ceth.open(fiveHundreduUSD, uUSD, {
					value: tenETH,
					from: account1,
				});
			...
			it('should set the loan correctly', async () => {
				...
				assert.equal(loan.currency, uUSD);
				assert.equal(loan.amount, fiveHundreduUSD.toString());
			...
			it('should issue the correct amount to the borrower', async () => {
				...
				assert.bnEqual(await uUSDSynth.balanceOf(account1), expectedBal);
			});

			it('should issue the minting fee to the fee pool', async () => {
				const feePoolBalance = await uUSDSynth.balanceOf(FEE_ADDRESS);
	...
	describe('withdraws', async () => {
		beforeEach(async () => {
			loan = await ceth.open(oneHundreduUSD, uUSD, {
				value: tenETH,
				from: account1,
			});
	...
	describe('repayments', async () => {
			...
			['System', 'Issuance'].forEach(section => {
				describe(`when ${section} is suspended`, () => {
					...
					it('then calling repay() reverts', async () => {
						await assert.revert(
							ceth.repay(account1, id, oneuUSD, { from: account1 }),
							'Operation prohibited'
						);
					});
					describe(`when ${section} is resumed`, () => {
						...
						it('then calling repay() succeeds', async () => {
							await ceth.repay(account1, id, oneuUSD, { from: account1 });
						});
					});
				});
			});
		...
		describe('revert conditions', async () => {
			...
			// account 2 had no uUSD
			it('should revert if they have no uUSD', async () => {
				await assert.revert(
					ceth.repay(account1, id, tenuUSD, { from: account2 }),
					'Not enough balance'
				);
			});

			it('should revert if they try to pay more than the amount owing', async () => {
				await issueuUSDToAccount(toUnit(1000), account1);
				await assert.revert(
		...
		describe('should allow repayments on an uUSD loan', async () => {
			// I'm not testing interest here, just that payment reduces the amounts.
			const expectedString = '90000';

			beforeEach(async () => {
				await issueuUSDToAccount(oneHundreduUSD, account2);
				tx = await ceth.repay(account1, id, tenuUSD, { from: account2 });
				loan = await ceth.loans(id);
			});

			it('should work reduce the repayers balance', async () => {
				const expectedBalance = toUnit(90);
				assert.bnEqual(await uUSDSynth.balanceOf(account2), expectedBalance);
			});
	...
	describe('liquidations', async () => {
		...
		beforeEach(async () => {
			// make a loan here so we have a valid ID to pass to the blockers and reverts.
			loan = await ceth.open(toUnit('200'), uUSD, {
				value: toUnit('2.6'),
				from: account1,
			});
		...
		describe('potential blocking conditions', async () => {
			['System', 'Issuance'].forEach(section => {
				describe(`when ${section} is suspended`, () => {
					...
					it('then calling repay() reverts', async () => {
						await assert.revert(
							ceth.liquidate(account1, id, oneuUSD, { from: account1 }),
							'Operation prohibited'
						);
					});
					describe(`when ${section} is resumed`, () => {
		...
		describe('revert conditions', async () => {
			it('should revert if they have no uUSD', async () => {
				await assert.revert(
					ceth.liquidate(account1, id, oneuUSD, { from: account2 }),
					'Not enough balance'
				);
			});

			it('should revert if they are not under collateralised', async () => {
				await issueuUSDToAccount(toUnit(100), account2);
		...
		describe('should allow liquidations on an undercollateralised uUSD loan', async () => {
			...
			beforeEach(async () => {
				...
				await issueuUSDToAccount(toUnit(1000), account2);
			...
			it('should reduce the liquidators synth amount', async () => {
				const liquidatorBalance = await uUSDSynth.balanceOf(account2);
			...
			it('should pay the interest to the fee pool', async () => {
				const balance = await uUSDSynth.balanceOf(FEE_ADDRESS);
				...
				describe(`when ${section} is suspended`, () => {
					...
					describe(`when ${section} is resumed`, () => {
						...
						it('then calling close() succeeds', async () => {
							// Give them some more uUSD to make up for the fees.
							await issueuUSDToAccount(tenuUSD, account1);
	...
	describe('drawing', async () => {
		...
		describe('potential blocking conditions', async () => {
			['System', 'Issuance'].forEach(section => {
				describe(`when ${section} is suspended`, () => {
					...
					it('then calling draw() reverts', async () => {
						await assert.revert(ceth.draw(id, oneuUSD, { from: account1 }), 'Operation prohibited');
					});
					describe(`when ${section} is resumed`, () => {
						beforeEach(async () => {
							await setStatus({ owner, systemStatus, section, suspend: false });
			...
			describe('when rates have gone stale', () => {
				...
				it('then calling draw() reverts', async () => {
					await assert.revert(ceth.draw(id, oneuUSD, { from: account1 }), 'Invalid rate');
				});
				describe('when ETH gets a rate', () => {
					beforeEach(async () => {
						await updateRatesWithDefaults();
					...
					it('then calling draw() succeeds', async () => {
						await ceth.draw(id, oneuUSD, { from: account1 });
					});
				});
			});
		});
		...
		describe('revert conditions', async () => {
			it('should revert if the draw would under collateralise the loan', async () => {
				await assert.revert(ceth.draw(id, oneHundreduUSD, { from: account1 }), 'Cratio too low');
			});
		});
