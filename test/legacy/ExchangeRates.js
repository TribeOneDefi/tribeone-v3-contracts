...
contract('Exchange Rates', async accounts => {
	...
	const [HAKA, sJPY, sETH, sXTZ, sBNB, uUSD, sEUR, sAUD, GOLD, fastGasPrice, debtRatio] = [
		'HAKA',
		'sJPY',
		'sETH',
		'sXTZ',
	...
	const itIsConstructedCorrectly = contract => {
		describe('constructor', () => {
			...
			it('returns correct values for uUSD after deployment ', async () => {
				assert.bnEqual(await instance.rateForCurrency(uUSD), toUnit('1'));
				assert.equal(await instance.lastRateUpdateTimes(uUSD), 0);
			});
		});
	};
	...
	const itCalculatesStaleRates = () => {
		...
		describe('rateIsStale()', () => {
			it('should never allow uUSD to go stale via rateIsStale', async () => {
				await fastForward(await instance.rateStalePeriod());
				const rateIsStale = await instance.rateIsStale(uUSD);
			...
			it('make sure anyone can check if rate is stale', async () => {
				await instance.rateIsStale(uUSD, { from: oracle });
				await instance.rateIsStale(uUSD, { from: owner });
				await instance.rateIsStale(uUSD, { from: deployerAccount });
				await instance.rateIsStale(uUSD, { from: accountOne });
				await instance.rateIsStale(uUSD, { from: accountTwo });
			});
		});
	};
		...
		describe('anyRateIsInvalid()', () => {
			describe('stale scenarios', () => {
				it('anyRateIsInvalid conforms to rateStalePeriod', async () => {
					await setupAggregators([HAKA, GOLD]);

					await updateRates([HAKA, GOLD], [toUnit(0.1), toUnit(0.2)]);

					assert.equal(await instance.anyRateIsInvalid([HAKA, GOLD]), false);

					await fastForward(await instance.rateStalePeriod());
					assert.equal(await instance.anyRateIsInvalid([HAKA, GOLD]), true);
				...
				describe('when sJPY aggregator is added', () => {
					...
					describe('when aggregated synth has rates', () => {
						...
						it('then rateIsInvalid is false', async () => {
							const rateIsInvalid = await instance.anyRateIsInvalid([sJPY, uUSD]);
							assert.equal(rateIsInvalid, false);
						});
		...
		describe('effectiveValue() and effectiveValueAndRates()', () => {
			describe('when a price is sent to the oracle', () => {
				...
				it('should correctly calculate an exchange rate in effectiveValue()', async () => {
					// 1 uUSD should be worth 2 sAUD.
					assert.bnEqual(await instance.effectiveValue(uUSD, toUnit('1'), sAUD), toUnit('2'));

					// 10 HAKA should be worth 1 uUSD.
					assert.bnEqual(await instance.effectiveValue(HAKA, toUnit('10'), uUSD), toUnit('1'));

					// 2 sEUR should be worth 2.50 uUSD
					assert.bnEqual(await instance.effectiveValue(sEUR, toUnit('2'), uUSD), toUnit('2.5'));
				});
				...
				it('should calculate updated rates in effectiveValue()', async () => {
					...
					// Update all rates except uUSD.
					await updateRates([sEUR, HAKA], ['1.25', '0.1'].map(toUnit));

					const amountOfTribeOnes = toUnit('10');
					const amountOfEur = toUnit('0.8');

					// Should now be able to convert from HAKA to sEUR since they are both not stale.
					assert.bnEqual(await instance.effectiveValue(HAKA, amountOfTribeOnes, sEUR), amountOfEur);
				});

				it('should return 0 when relying on a non-existant dest exchange rate in effectiveValue()', async () => {
					assert.equal(await instance.effectiveValue(HAKA, toUnit('10'), toBytes32('XYZ')), 0);
				});

				it('should revert when relying on a non-existing src rate in effectiveValue', async () => {
					assert.equal(await instance.effectiveValue(toBytes32('XYZ'), toUnit('10'), HAKA), 0);
				});

				it('effectiveValueAndRates() should return rates as well with uUSD on one side', async () => {
				...
				it('effectiveValueAndRates() should return rates as well with uUSD on the other side', async () => {
					const { value, sourceRate, destinationRate } = await instance.effectiveValueAndRates(
						sAUD,
						toUnit('1'),
						uUSD
	...
	const itReadsFromAggregator = () => {
		describe('when the flags interface is set', () => {
			...
			describe('pricing aggregators', () => {
						...
						describe('when the aggregator price is set for sJPY', () => {
							...
							describe('when the aggregator price is set for sXTZ', () => {
								...
								describe('when the ratesAndInvalidForCurrencies is queried', () => {
									let response;
									beforeEach(async () => {
										response = await instance.ratesAndInvalidForCurrencies([sJPY, sXTZ, uUSD]);
									});
								...
								describe('when rateAndInvalid is queried', () => {
									...
									beforeEach(async () => {
										responseJPY = await instance.rateAndInvalid(sJPY);
										responseXTZ = await instance.rateAndInvalid(sXTZ);
										responseUSD = await instance.rateAndInvalid(uUSD);
									});
								...
								describe('when the aggregator is removed for sJPY', () => {
									...
									it('when the ratesAndInvalidForCurrencies is queried it returns 0', async () => {
										assert.deepEqual(
											await instance.ratesAndInvalidForCurrencies([sJPY, sXTZ, uUSD]),
									...
									describe('when rateAndInvalid is queried', () => {
										...
										it('other rates are fine', async () => {
											...
											const responseUSD = await instance.rateAndInvalid(uUSD);
				...
				describe('warning flags and invalid rates', () => {
					it('uUSD is never flagged / invalid.', async () => {
						assert.isFalse(await instance.rateIsFlagged(uUSD));
						assert.isFalse(await instance.rateIsInvalid(uUSD));
					});
		...
		describe('roundIds for historical rates', () => {
			...
			it('getCurrentRoundId() is 0 for uUSD', async () => {
				assert.equal(await instance.getCurrentRoundId(uUSD), 0);
			});

			it('ratesAndUpdatedTimeForCurrencyLastNRounds() shows first entry for uUSD', async () => {
				assert.deepEqual(await instance.ratesAndUpdatedTimeForCurrencyLastNRounds(uUSD, '3', '0'), [
					[toUnit('1'), '0', '0'],
					[0, 0, 0],
				]);
			});
	...
	const itReadsAtomicPricesFromDex = () => {
		...
		describe('src/dest do not have an atomic equivalent for dex pricing', () => {
			beforeEach(async () => {
				...
				// set sETH equivalent but don't set uUSD equivalent
			...
			it('reverts on src not having equivalent', async () => {
				await assert.revert(
					instance.effectiveAtomicValueAndRates(uUSD, toUnit('1'), sETH),
					'No atomic equivalent for source'
				);
			});
			it('reverts on dest not having equivalent', async () => {
				await assert.revert(
					instance.effectiveAtomicValueAndRates(sETH, toUnit('1'), uUSD),
		...
		describe('effectiveAtomicValueAndRates', () => {
			...
			function itGivesTheCorrectRates({
				...
				describe(`P_DEX of ${pDex}, P_CL of ${pClRaw}`, () => {
					...
					const pClInUsdIn8 = srcToken === uUSD ? divideDecimal(unitIn8, pClIn8, unitIn8) : pClIn8;
					...
					it('provides the correct system source rate', () => {
						if (srcToken === uUSD) {
							assert.bnEqual(rates.systemSourceRate, one); // uUSD is always one
						} else {
					...
					it('provides the correct system destination rate', () => {
						if (destToken === uUSD) {
							assert.bnEqual(rates.systemDestinationRate, one); // uUSD is always one
						} else {
			...
			beforeEach('set up mocks', async () => {
				...
				susdDexEquivalentToken = await MockToken.new('euUSD equivalent', 'euUSD', '18');
			...
			describe('dexPriceAggregator reverts on assetToAsset', () => {
				...
				it('reverts', async () => {
					await assert.revert(
						instance.effectiveAtomicValueAndRates(uUSD, one, sETH),
						'mock assetToAsset() reverted'
					);
				});
			});

			describe('trades uUSD -> sETH', () => {
			...
			describe('trades sETH -> uUSD', () => {
				const amountIn = toUnit('10');
				const srcToken = sETH;
				const destToken = uUSD;
			...
			describe('when tokens use non-18 decimals', () => {
				beforeEach('set up non-18 decimal tokens', async () => {
					susdDexEquivalentToken = await MockToken.new('uUSD equivalent', 'euUSD', '6'); // mimic USDC and USDT
				...
				describe('uUSD -> sETH', () => {
					...
					xit('still provides results in 18 decimals', async () => {
						const rates = await instance.effectiveAtomicValueAndRates(uUSD, amountIn, sETH);
				...
				describe('sETH -> uUSD', () => {
					...
					it('still provides results in 18 decimals', async () => {
						const rates = await instance.effectiveAtomicValueAndRates(sETH, amountIn, uUSD);
	...
	const itDoesntReadAtomicPricesFromDex = () => {
		describe('Atomic exchange pricing', () => {
			it('errors with not implemented when attempting to fetch atomic rate', async () => {
				await assert.revert(
					instance.effectiveAtomicValueAndRates(sETH, toUnit('10'), uUSD),
	...
	describe('Using ExchangeRates', () => {
		...
		before(async () => {
			...
			await instance.removeAggregator(toBytes32('HAKA'), { from: owner });
