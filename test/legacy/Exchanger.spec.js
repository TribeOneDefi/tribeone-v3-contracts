...
contract('Exchanger (spec tests)', async accounts => {
	const [uUSD, sAUD, sEUR, HAKA, sBTC, iBTC, sETH, iETH] = [
		'uUSD',
		'sAUD',
		'sEUR',
		'HAKA',
	...
	const synthKeys = [uUSD, sAUD, sEUR, sBTC, iBTC, sETH, iETH];
	...
	async function changeOneDISetting(
		index,
		value,
		synths = [uUSD, sAUD, sEUR, HAKA, sBTC, iBTC, sETH, iETH]
	) {
		...
		describe('waitingPeriodSecs', () => {
			...
			describe('given it is configured to 90', () => {
				...
				describe('and there is an exchange', () => {
					beforeEach(async () => {
						await tribeone.exchange(uUSD, toUnit('100'), sEUR, { from: account1 });
					});
		...
		describe('When the waiting period is set to 0', () => {
			...
			describe('When exchanging', () => {
				...
				beforeEach(async () => {
					...
					await uUSDContract.issue(owner, toUnit('100'));
					await tribeone.exchange(uUSD, toUnit('10'), sETH, { from: owner });
				});
				...
				it('can exchange back without waiting', async () => {
					...
					await tribeone.exchange(sETH, amountReceived, uUSD, { from: owner });
				...
				describe('When the waiting period is switched on again', () => {
					...
					describe('a new exchange takes place', () => {
						...
						beforeEach(async () => {
							...
							exchangeTransaction = await tribeone.exchange(uUSD, amountOfSrcExchanged, sETH, {
								from: owner,
							});
						});
						...
						it('then it emits an ExchangeEntryAppended', async () => {
							...
							const logs = await getDecodedLogs({
								...
								contracts: [tribeone, exchanger, uUSDContract, issuer, flexibleStorage, debtCache],
							});
							decodedEventEqual({
						...
						describe('When the waiting period is set back to 0', () => {
							...
							describe('new trades take place', () => {
								beforeEach(async () => {
									...
									await tribeone.exchange(sETH, sEthBalance, uUSD, { from: owner });
									await tribeone.exchange(uUSD, toUnit('10'), sEUR, { from: owner });
								});
		...
		describe('priceDeviationThresholdFactor()', () => {
			...
			describe('when a user exchanges into sETH over the default threshold factor', () => {
				...
				beforeEach(async () => {
					...
					const { tx: hash } = await tribeone.exchange(uUSD, toUnit('1'), sETH, {
						from: account1,
					});
				...
				describe('when the factor is set to 3.1', () => {
					...
					describe('when a user exchanges into sETH over the default threshold factor, but under the new one', () => {
						beforeEach(async () => {
							...
							// base rate of sETH is 100 from shared setup above
							await updateRates([sETH], [toUnit('300')]);
							await tribeone.exchange(uUSD, toUnit('1'), sETH, { from: account1 });
						});
	...
	const itCalculatesMaxSecsLeft = () => {
		describe('maxSecsLeftInWaitingPeriod()', () => {
			describe('when the waiting period is configured to 60', () => {
				...
				describe('when a user with uUSD has performed an exchange into sEUR', () => {
					beforeEach(async () => {
						await tribeone.exchange(uUSD, toUnit('100'), sEUR, { from: account1 });
					});
					...
					it('and fetching maxSecs for that user into the source synth returns 0', async () => {
						const maxSecs = await exchanger.maxSecsLeftInWaitingPeriod(account1, uUSD);
					...
					it('and fetching maxSec for other users into that synth are unaffected', async () => {
						...
						maxSecs = await exchanger.maxSecsLeftInWaitingPeriod(account2, uUSD);
						assert.equal(
							maxSecs,
							'0',
					...
					describe('when 55 seconds has elapsed', () => {
						...
						describe('when another user does the same exchange', () => {
							beforeEach(async () => {
								await tribeone.exchange(uUSD, toUnit('100'), sEUR, { from: account2 });
							});
		...
		describe('given exchange fee rates are configured into categories', () => {
			...
			beforeEach(async () => {
				await setExchangeFeeRateForSynths({
					owner,
					systemSettings,
					synthKeys: [uUSD, sAUD, sEUR, sETH, sBTC, iBTC],
					exchangeFeeRates: [bipuUSD, bipsFX, bipsFX, bipsCrypto, bipsCrypto, bipsInverse],
				});
			});
			describe('when calling getAmountsForExchange', () => {
				describe('and the destination is a crypto synth', () => {
					...
					let feeRate;
					beforeEach(async () => {
						await tribeone.exchange(uUSD, amountIssued, sBTC, { from: account1 });
						const {
							amountReceived,
							fee,
							exchangeFeeRate,
						} = await exchanger.getAmountsForExchange(amountIssued, uUSD, sBTC, { from: account1 });
					...
					it('then return the fee', async () => {
						const effectiveValue = await exchangeRates.effectiveValue(uUSD, amountIssued, sBTC);
						assert.bnEqual(destinationFee, exchangeFeeIncurred(effectiveValue, bipsCrypto));
					});
					it('then return the feeRate', async () => {
						const exchangeFeeRate = await exchanger.feeRateForExchange(uUSD, sBTC, {
							from: account1,
						});
				...
				describe('and the destination is a fiat synth', () => {
					...
					let feeRate;
					beforeEach(async () => {
						await tribeone.exchange(uUSD, amountIssued, sEUR, { from: account1 });
						const {
							amountReceived,
							fee,
							exchangeFeeRate,
						} = await exchanger.getAmountsForExchange(amountIssued, uUSD, sEUR, { from: account1 });
						received = amountReceived;
						destinationFee = fee;
						feeRate = exchangeFeeRate;
					});
					...
					it('then return the fee', async () => {
						const effectiveValue = await exchangeRates.effectiveValue(uUSD, amountIssued, sEUR);
						assert.bnEqual(destinationFee, exchangeFeeIncurred(effectiveValue, bipsFX));
					});
					it('then return the feeRate', async () => {
						const exchangeFeeRate = await exchanger.feeRateForExchange(uUSD, sEUR, {
							from: owner,
						});
						assert.bnEqual(feeRate, exchangeFeeRate);
					});
				...
				describe('when the fees are changed', () => {
					...
					it('updates exchange fee amounts appropriately', async () => {
						...
						} = await exchanger.getAmountsForExchange(amount, uUSD, sAUD, { from: account1 });
				...
				describe('when tripling an exchange rate', () => {
					...
					it('then return the fee tripled', async () => {
						const { fee } = await exchanger.getAmountsForExchange(amount, uUSD, sAUD, {
							from: owner,
						});
						assert.bnEqual(fee, multiplyDecimal(orgininalFee, factor));
					});
					it('then return the feeRate tripled', async () => {
						const { exchangeFeeRate } = await exchanger.getAmountsForExchange(amount, uUSD, sAUD, {
							from: owner,
						});
					...
					it('then return the amountReceived less triple the fee', async () => {
						const { amountReceived } = await exchanger.getAmountsForExchange(amount, uUSD, sAUD, {
							from: owner,
						});
						const tripleFee = multiplyDecimal(orgininalFee, factor);
						const effectiveValue = await exchangeRates.effectiveValue(uUSD, amount, sAUD);
				...
				describe('dynamic fee when rates change', () => {
					...
					it('initial fee is correct', async () => {
						assert.bnEqual(
							await exchanger.feeRateForExchange(uUSD, sBTC, { from: owner }),
							bipsCrypto
						);
						assert.deepEqual(
							await exchanger.dynamicFeeRateForExchange(uUSD, sBTC, { from: owner }),
							[0, false]
						);
					});
					...
					describe('fee is calculated correctly when rates spike or drop', () => {
						it('.3% spike is below threshold', async () => {
							...
							// spike
							assert.bnEqual(
								await exchanger.feeRateForExchange(uUSD, sETH, { from: owner }),
								bipsCrypto
							);
							assert.deepEqual(
								await exchanger.dynamicFeeRateForExchange(uUSD, sETH, { from: owner }),
								[0, false]
							);
							// control
							assert.bnEqual(
						...
						it('10% spike is over the max and is too volatile', async () => {
							...
							await assert.revert(
								exchanger.feeRateForExchange(uUSD, sETH, { from: owner }),
								'too volatile'
							);
							assert.deepEqual(
								await exchanger.dynamicFeeRateForExchange(uUSD, sETH, { from: owner }),
						...
						it('10% drop result in correct dynamic fee', async () => {
							...
							// view reverts
							await assert.revert(
								exchanger.getAmountsForExchange(toUnit('1'), uUSD, sETH, { from: owner }),
								'too volatile'
							);
							// control
							assert.bnEqual(
					...
					it('no exchange happens when dynamic fee is too high', async () => {
						...
						async function echangeSuccessful() {
							// this should work
							const txn = await tribeone.exchange(sETH, toUnit('1'), uUSD, { from: account1 });
							const logs = await getDecodedLogs({
								hash: txn.tx,
								contracts: [tribeone, exchanger, systemStatus],
							});
						...
						// view doesn't revert
						await exchanger.getAmountsForExchange(toUnit('1'), sETH, uUSD, { from: account1 });
						...
						// view reverts
						await assert.revert(
							exchanger.getAmountsForExchange(toUnit('1'), sETH, uUSD, { from: account1 }),
							'too volatile'
						);
					});
	...
	const ensureTxnEmitsSettlementEvents = async ({ hash, synth, expected }) => {
		...
		const logs = await getDecodedLogs({ hash, contracts: [tribeone, exchanger, uUSDContract] });
			...
			describe('suspension conditions', () => {
				...
				['System', 'Synth'].forEach(section => {
					describe(`when ${section} is suspended`, () => {
						...
						it('then calling settle() reverts', async () => {
							await assert.revert(
								tribeone.settle(sETH, { from: account1 }),
						...
						describe(`when ${section} is resumed`, () => {
							...
							it('then calling exchange() succeeds', async () => {
								await tribeone.settle(sETH, { from: account1 });
							});
						});
					});
				});
				...
				describe('when Synth(sBTC) is suspended for exchanging', () => {
					...
					it('then settling it still works', async () => {
						await tribeone.settle(sBTC, { from: account1 });
					});
				});
			});
			describe('given the sEUR rate is 2, and sETH is 100, sBTC is 9000', () => {
				beforeEach(async () => {
					// set uUSD:sEUR as 2:1, uUSD:sETH at 100:1, uUSD:sBTC at 9000:1
						...
						describe('various rebate & reclaim scenarios', () => {
							describe('when the debt cache is replaced with a spy', () => {
								...
								describe('and the priceDeviationThresholdFactor is set to a factor of 2.5', () => {
									...
									describe('when the first user exchanges 100 uUSD into uUSD:sEUR at 2:1', () => {
										...
										beforeEach(async () => {
											...
											const {
												amountReceived,
												exchangeFeeRate,
											} = await exchanger.getAmountsForExchange(amountOfSrcExchanged, uUSD, sEUR, {
												from: account1,
											});
										...
										describe('when the waiting period elapses', () => {
											...
											describe('when settle() is invoked on sEUR', () => {
												...
												it('then it completes with one settlement', async () => {
													const logs = await getDecodedLogs({
														hash: txn.tx,
														contracts: [tribeone, exchanger, uUSDContract],
													});
										...
										it('when sEUR is attempted to be exchanged away by the user, it reverts', async () => {
											await assert.revert(
												tribeone.exchange(sEUR, toUnit('1'), sBTC, { from: account1 }),
												'Cannot settle during waiting period'
											);
										});

										describe('when settle() is invoked on the src synth - uUSD', () => {
											it('then it completes with no reclaim or rebate', async () => {
												const txn = await tribeone.settle(uUSD, {
													from: account1,
												});
												assert.equal(
													txn.logs.length,
										...
										describe('when the price doubles for uUSD:sEUR to 4:1', () => {
											beforeEach(async () => {
												await fastForward(5);
											...
											describe('when another minute passes', () => {
												...
												describe(
													...
													() => {
														let txn;
														beforeEach(async () => {
															txn = await tribeone.exchange(sEUR, toUnit('30'), sBTC, {
																from: account1,
															});
														});
												...
												describe(
													...
													() => {
														let txn;
														beforeEach(async () => {
															txn = await tribeone.exchange(sEUR, toUnit('50'), sBTC, {
																from: account1,
															});
															await setExchangeFeeRateForSynths({
																owner,
												...
												describe('when an exchange out of sEUR for less than the balance after settlement', () => {
													...
													beforeEach(async () => {
														...
														txn = await tribeone.exchange(sEUR, newAmountToExchange, sBTC, {
															from: account1,
														});
													});
										...
										describe('when the price halves for uUSD:sEUR to 1:1', () => {
											beforeEach(async () => {
												await fastForward(5);
											...
											describe('when the user makes a 2nd exchange of 100 uUSD into uUSD:sEUR at 1:1', () => {
												beforeEach(async () => {
													// fast forward 60 seconds so 1st exchange is using first rate
													await fastForward(60);

													await tribeone.exchange(uUSD, amountOfSrcExchanged, sEUR, {
														from: account1,
													});
												});

												describe('and then the price increases for uUSD:sEUR to 2:1', () => {
													beforeEach(async () => {
														await fastForward(5);
											...
											describe('when settlement is invoked', () => {
												...
												describe('when another minute passes', () => {
													...
													describe('when an exchange out of sEUR for their expected balance before exchange', () => {
														let txn;
														beforeEach(async () => {
															txn = await tribeone.exchange(sEUR, toUnit('49'), sBTC, {
																from: account1,
															});
														});
													...
													describe('when an exchange out of sEUR for some amount less than their balance before exchange', () => {
														let txn;
														beforeEach(async () => {
															txn = await tribeone.exchange(sEUR, toUnit('10'), sBTC, {
																from: account1,
															});
														});
											...
											describe('when the price returns to uUSD:sEUR to 2:1', () => {
												beforeEach(async () => {
									...
									describe('given the first user has 1000 sEUR', () => {
										...
										describe('when the first user exchanges 100 sEUR into sEUR:sBTC at 9000:2', () => {
											...
											beforeEach(async () => {
												amountOfSrcExchanged = toUnit('100');
												await tribeone.exchange(sEUR, amountOfSrcExchanged, sBTC, {
													from: account1,
												});
											});
											it('then settlement reclaimAmount shows 0 reclaim and 0 refund', async () => {
											...
											describe('when the price doubles for uUSD:sEUR to 4:1', () => {
												...
												describe('when settlement is invoked', () => {
													it('then it reverts as the waiting period has not ended', async () => {
														await assert.revert(
															tribeone.settle(sBTC, { from: account1 }),
															'Cannot settle during waiting period'
														);
													});
												});
												describe('when the price gains for sBTC more than the loss of the sEUR change', () => {
													...
													describe('when the same user exchanges some uUSD into sBTC - the same destination', () => {
														let amountOfSrcExchangedSecondary;
														beforeEach(async () => {
															amountOfSrcExchangedSecondary = toUnit('10');
															await tribeone.exchange(uUSD, amountOfSrcExchangedSecondary, sBTC, {
																from: account1,
															});
														});
															...
															describe('when another minute passes', () => {
																...
																describe('when settle() is invoked for sBTC', () => {
																	it('then it settles with a rebate @gasprofile', async () => {
																		const txn = await tribeone.settle(sBTC, {
																			from: account1,
																		});
															...
															describe('when another minute passes and the exchange fee rate has increased', () => {
																...
																describe('when settle() is invoked for sBTC', () => {
																	it('then it settles with a rebate using the exchange fee rate at time of trade', async () => {
																		const { tx: hash } = await tribeone.settle(sBTC, {
																			from: account1,
																		});
										...
										describe('and the max number of exchange entries is 5', () => {
											...
											describe('when a user tries to exchange 100 sEUR into sBTC 5 times', () => {
												beforeEach(async () => {
													...
													for (let i = 0; i < 5; i++) {
														txns.push(
															await tribeone.exchange(sEUR, toUnit('100'), sBTC, {
																from: account1,
															})
														);
													}
												...
												it('when one more is tried, then if fails', async () => {
													await assert.revert(
														tribeone.exchange(sEUR, toUnit('100'), sBTC, { from: account1 }),
		...
		describe('exchange()', () => {
			it('exchange() cannot be invoked directly by any account', async () => {
				await onlyGivenAddressCanInvoke({
					...
					reason: 'Only tribeone or a synth contract can perform this action',
				});
			});
			...
			describe('suspension conditions on TribeOne.exchange()', () => {
				...
				['System', 'Exchange', 'SynthExchange', 'Synth'].forEach(section => {
					describe(`when ${section} is suspended`, () => {
						...
						it('then calling exchange() reverts', async () => {
							await assert.revert(
								tribeone.exchange(uUSD, toUnit('1'), sETH, { from: account1 }),
								'Operation prohibited'
							);
						});
						describe(`when ${section} is resumed`, () => {
				...
				describe('when Synth(sBTC) is suspended', () => {
					beforeEach(async () => {
						// issue sAUD to test non-uUSD exchanges
					...
					it('then exchanging other synths still works', async () => {
						await tribeone.exchange(uUSD, toUnit('1'), sETH, { from: account1 });
						await tribeone.exchange(sAUD, toUnit('1'), sETH, { from: account2 });
					});
				});
			});

			describe('various exchange scenarios', () => {
				describe('when a user has 1000 uUSD', () => {
					...
					it('should allow a user to exchange the synths they hold in one flavour for another', async () => {
						// Exchange uUSD to sAUD
						await tribeone.exchange(uUSD, amountIssued, sAUD, { from: account1 });
						...
						} = await exchanger.getAmountsForExchange(amountIssued, uUSD, sAUD, {
							from: account1,
						});
						...
						const usdFeeAmount = await exchangeRates.effectiveValue(sAUD, fee, uUSD);
						assert.bnEqual(usdFeeAmount, feePeriodZero.feesToDistribute);

						// Double the exchange fee rate (once for uUSD, once for sAUD)
						assert.bnEqual(feeRate, exchangeFeeRate.add(exchangeFeeRate));
					});

					it('should emit a SynthExchange event @gasprofile', async () => {
						// Exchange uUSD to sAUD
						const txn = await tribeone.exchange(uUSD, amountIssued, sAUD, {
							from: account1,
						});
						...
						assert.eventEqual(synthExchangeEvent, 'SynthExchange', {
							account: account1,
							fromCurrencyKey: toBytes32('uUSD'),
					...
					it('should emit an ExchangeTracking event @gasprofile', async () => {
						...
						const { fee } = await exchanger.getAmountsForExchange(amountIssued, uUSD, sAUD, {
							from: account1,
						});
						const usdFeeAmount = await exchangeRates.effectiveValue(sAUD, fee, uUSD);
					...
					it('when a user tries to exchange more than they have, then it fails', async () => {
						await assert.revert(
							tribeone.exchange(sAUD, toUnit('1'), uUSD, {
								from: account1,
							}),
					...
					it('when a user tries to exchange more than they have, then it fails', async () => {
						await assert.revert(
							tribeone.exchange(uUSD, toUnit('1001'), sAUD, {
								from: account1,
							}),
							'SafeMath: subtraction overflow'
						);
					...
					[
						...
						describe(`rate stale scenarios for ${type}`, () => {
							...
							describe('when rates have gone stale for all synths', () => {
								...
								it(`attempting to ${type} from uUSD into sAUD reverts with dest stale`, async () => {
									await assert.revert(
										exchange({ from: uUSD, amount: amountIssued, to: sAUD }),
										'dest rate stale or flagged'
									);
									// view reverts
									await assert.revert(
										exchanger.getAmountsForExchange(toUnit('1'), uUSD, sAUD),
										'invalid'
									);
								});
								...
								describe('when that synth has a fresh rate', () => {
									...
									describe(`when the user ${type} into that synth`, () => {
										beforeEach(async () => {
											await exchange({ from: uUSD, amount: amountIssued, to: sAUD });
										});
										describe('after the waiting period expires and the synth has gone stale', () => {
											...
											it(`${type} back to uUSD fails as the source has no rate`, async () => {
												await assert.revert(
													exchange({ from: sAUD, amount: amountIssued, to: uUSD }),
					...
					describe('exchanging on behalf', async () => {
						...
						describe('when not approved it should revert on', async () => {
							it('exchangeOnBehalf', async () => {
								await assert.revert(
									tribeone.exchangeOnBehalf(authoriser, sAUD, toUnit('1'), uUSD, {
										from: delegate,
									}),
						...
						describe('when delegate address approved to exchangeOnBehalf', async () => {
							// (uUSD amount issued earlier in top-level beforeEach)
							beforeEach(async () => {
							...
							describe('suspension conditions on TribeOne.exchangeOnBehalf()', () => {
								...
								['System', 'Exchange', 'SynthExchange', 'Synth'].forEach(section => {
									describe(`when ${section} is suspended`, () => {
										...
										it('then calling exchange() reverts', async () => {
											await assert.revert(
												tribeone.exchangeOnBehalf(authoriser, uUSD, amountIssued, sAUD, {
													from: delegate,
												}),
												'Operation prohibited'
											);
										...
										describe(`when ${section} is resumed`, () => {
											...
											it('then calling exchange() succeeds', async () => {
												await tribeone.exchangeOnBehalf(authoriser, uUSD, amountIssued, sAUD, {
													from: delegate,
												});
											});
										});
							...
							it('should revert if non-delegate invokes exchangeOnBehalf', async () => {
								await onlyGivenAddressCanInvoke({
									...
									args: [authoriser, uUSD, amountIssued, sAUD],
						...
						describe('when delegate address approved to exchangeOnBehalf', async () => {
							...
							it('should revert if non-delegate invokes exchangeOnBehalf', async () => {
								await onlyGivenAddressCanInvoke({
									...
									args: [authoriser, uUSD, amountIssued, sAUD, authoriser, trackingCode],
									// We cannot test the revert condition with the authoriser as the recipient
									// because this will lead to a regular exchange, not one on behalf
									accounts: accounts.filter(a => a !== authoriser),
									address: delegate,
				...
				describe('when an aggregator is added to the exchangeRates', () => {
					...
					describe('when exchanging into that synth', () => {
						it('getAmountsForExchange reverts due to invalid rate', async () => {
							await assert.revert(
								exchanger.getAmountsForExchange(toUnit('1'), uUSD, sETH),
								'synth rate invalid'
							);
						});
					...
					describe('when exchanging out of that synth', () => {
						...
						it('getAmountsForExchange reverts due to invalid rate', async () => {
							await assert.revert(
								exchanger.getAmountsForExchange(toUnit('1'), sETH, uUSD),
								'synth rate invalid'
							);
						});
						it('then it causes a breaker trip from price deviation', async () => {
							// await assert.revert(
							const { tx: hash } = await tribeone.exchange(sETH, toUnit('1'), uUSD, {
								from: account1,
							});
					...
					describe('when a user exchanges into sAUD using virtual synths with a tracking code', () => {
						...
						beforeEach(async () => {
							...
							({
								amountReceived,
								exchangeFeeRate,
							} = await exchanger.getAmountsForExchange(amountIssued, uUSD, sAUD, { from: owner }));
						...
						it('then it emits an SynthExchange into the new Virtual Synth', async () => {
							decodedEventEqual({
								...
								event: 'SynthExchange',
								emittedFrom: await tribeone.proxy(),
								args: [account1, uUSD, amountIssued, sAUD, amountReceived, vSynthAddress],
								bnCloseVariance: '0',
							});
						});
	...
	const itFailsToExchangeWithVirtual = () => {
		describe('it cannot use exchangeWithVirtual()', () => {
			it('errors with not implemented when attempted to exchange', async () => {
				await assert.revert(
					tribeone.exchangeWithVirtual(uUSD, amountIssued, sAUD, toBytes32(), {
						from: account1,
					}),
		...
		describe('exchangeAtomically()', () => {
			...
			describe('when a user has 1000 uUSD', () => {
				describe('when the necessary configuration been set', () => {
					...
					beforeEach(async () => {
						...
						const susdDexEquivalentToken = await MockToken.new('euUSD equivalent', 'euUSD', '18');
						const sethDexEquivalentToken = await MockToken.new('esETH equivalent', 'esETH', '18');
						await setAtomicEquivalentForDexPricing(uUSD, susdDexEquivalentToken.address);
					...
					describe('when a user sets a minimum amount', () => {
						...
						it('reverts when the received amount is too low', async () => {
							await assert.revert(
								tribeone.exchangeAtomically(uUSD, amountIn, sETH, toBytes32(), toUnit('.498'), {
									from: account1,
								}),
						...
						it('succeeds when the received amount is equal to the minimum amount', async () => {
							...
							assert.bnEqual(await uUSDContract.balanceOf(account1), amountIssued.sub(amountIn));
					...
					describe('when the user exchanges into sETH using an atomic exchange with a tracking code', () => {
						...
						beforeEach(async () => {
							...
							({
								...
								exchangeFeeRate,
								fee: amountFee,
							} = await exchanger.getAmountsForAtomicExchange(amountIn, uUSD, sETH, {
								from: account1,
							}));
						...
						it('used correct fee rate', async () => {
							const expectedFeeRate = await exchanger.feeRateForAtomicExchange(uUSD, sETH, {
								from: account1,
							});
						...
						it('emits an SynthExchange directly to the user', async () => {
							decodedEventEqual({
								...
								event: 'SynthExchange',
								emittedFrom: await tribeone.proxy(),
								args: [account1, uUSD, amountIn, sETH, amountReceived, account1],
								bnCloseVariance: '0',
							});
						});
						...
						it('emits an ExchangeTracking event with the correct code', async () => {
							const usdFeeAmount = await exchangeRates.effectiveValue(sETH, amountFee, uUSD);
							decodedEventEqual({
					...
					describe('when a fee override has been set for atomic exchanges', () => {
						...
						beforeEach(async () => {
							await tribeone.exchangeAtomically(uUSD, amountIn, sETH, toBytes32(), 0, {
								from: account1,
							});
					...
					describe('when a user exchanges without a tracking code', () => {
						let txn;
						beforeEach(async () => {
							txn = await tribeone.exchangeAtomically(uUSD, toUnit('10'), sETH, toBytes32(), 0, {
								from: account1,
							});
						});
						it('then no ExchangeTracking is emitted (as no tracking code supplied)', async () => {
			...
			describe('when we can use the pure Chainlink price', () => {
				...
				beforeEach(async () => {
					...
					const susdDexEquivalentToken = await MockToken.new('uUSD equivalent', 'euUSD', '6');
					...
					for (const forexCurrencyKey of [sAUD, sEUR, uUSD]) {
				...
				describe('for the source currency', () => {
					...
					beforeEach(async () => {
						...
						await tribeone.exchangeAtomically(sEUR, amountIn, sBTC, toBytes32(), 0, {
							from: account1,
						});
					});
				...
				describe('for the destination currency', () => {
					...
					beforeEach(async () => {
						...
						await tribeone.exchangeAtomically(sBTC, amountIn, sEUR, toBytes32(), 0, {
							from: account1,
						});
					});
	...
	const itFailsToExchangeAtomically = () => {
		describe('it cannot exchange atomically', () => {
			it('errors with not implemented when attempted to exchange', async () => {
				await assert.revert(
					tribeone.exchangeAtomically(uUSD, amountIssued, sETH, toBytes32(), 0, {
						from: account1,
					}),
					'Cannot be run on this layer'
				);
			...
			describe(`when the price of sETH is ${baseRate}`, () => {
				...
				describe('when price spike deviation is set to a factor of 2', () => {
						...
						describe('given the user has some sETH', () => {
							...
							const assertBothSidesOfTheExchange = () => {
								describe('on the dest side', () => {
									assertRange({ from: uUSD, to: sETH, target: sETH });
								});
							...
							describe('with a prior exchange from another user into the source', () => {
								beforeEach(async () => {
									await tribeone.exchange(uUSD, toUnit('1'), sETH, { from: account2 });
								});
							...
							describe('with a prior exchange from another user out of the source', () => {
								beforeEach(async () => {
									...
									await tribeone.exchange(sETH, toUnit('1'), sAUD, { from: account2 });
								});
					...
					describe('settlement ignores deviations', () => {
						...
						describe('when a user exchange 100 uUSD into sETH', () => {
							beforeEach(async () => {
								// Disable Dynamic Fee in settlement by setting rounds to 1
								await setExchangeDynamicFeeRounds('1');
								await tribeone.exchange(uUSD, toUnit('100'), sETH, { from: account1 });
							});
										...
										describe('and the user makes another exchange into sETH', () => {
											...
											describe('and the sETH rate goes back up 25% (from 80 to 100)', () => {
												...
												describe('and the waiting period expires', () => {
													...
													describe('and the user makes another exchange into sETH', () => {
														beforeEach(async () => {
															await tribeone.exchange(uUSD, toUnit('100'), sETH, {
																from: account1,
															});
														});
						...
						describe('edge case: aggregator returns 0 for settlement price', () => {
							describe('when an aggregator is added to the exchangeRates', () => {
								...
								describe('and the aggregator has a rate (so the exchange succeeds)', () => {
									...
									describe('when a user exchanges out of the aggregated rate into uUSD', () => {
										beforeEach(async () => {
											// give the user some sETH
											await sETHContract.issue(account1, toUnit('1'));
											await tribeone.exchange(sETH, toUnit('1'), uUSD, { from: account1 });
										});
										describe('and the aggregated rate becomes 0', () => {
											...
											it('then settlementOwing is 0 for rebate and reclaim, with 1 entry', async () => {
												const {
													...
													rebateAmount,
													numEntries,
												} = await exchanger.settlementOwing(account1, uUSD);
												assert.equal(reclaimAmount, '0');
												assert.equal(rebateAmount, '0');
												assert.equal(numEntries, '1');
											});
											describe('and the waiting period expires', () => {
												...
												it('then the user can settle with no impact', async () => {
													const txn = await exchanger.settle(account1, uUSD, { from: account1 });
									...
									describe('when a user exchanges into the aggregated rate from uUSD', () => {
										beforeEach(async () => {
											await tribeone.exchange(uUSD, toUnit('1'), sETH, { from: account1 });
										});
										describe('and the aggregated rate becomes 0', () => {
		...
		describe('Given synth exchange fee rates to set', async () => {
			...
			describe('Given synth exchange fee rates to update', async () => {
				...
				it('when 1 exchange rate to update then overwrite existing rate', async () => {
					...
					const uUSDRate = await exchanger.feeRateForExchange(empty, uUSD, { from: owner });
					assert.bnEqual(uUSDRate, newFxBIPS);
				});
	...
	describe('With L1 configuration (TribeOne, ExchangerWithFeeRecAlternatives, ExchangeRatesWithDexPricing)', () => {
		before(async () => {
			...
			const synths = ['uUSD', 'sETH', 'sEUR', 'sAUD', 'sBTC', 'iBTC', 'sTRX'];
			...
			// give the first two accounts 1000 uUSD each
			await uUSDContract.issue(account1, amountIssued);
			await uUSDContract.issue(account2, amountIssued);
		});

		addSnapshotBeforeRestoreAfterEach();

		beforeEach(async () => {
			const keys = [sAUD, sEUR, HAKA, sETH, sBTC, iBTC];
	...
	describe('With Direct Integration overrides configuration (TribeOne, ExchangerWithFeeRecAlternatives, ExchangeRatesWithDexPricing)', () => {
		...
		before('apply systemsettings & override ', async () => {
			...
			for (const token of [uUSD, sAUD, sEUR, HAKA, sBTC, iBTC, sETH, iETH]) {
