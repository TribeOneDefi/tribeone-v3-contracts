...
contract('CollateralEth', async accounts => {
	...
	describe('cratio test', async () => {
		describe('hUSD loans', async () => {
			beforeEach(async () => {
				tx = await ceth.open(oneHundredsUSD, hUSD, {
					value: twoETH,
					from: account1,
				});
	...
	describe('max loan test', async () => {
		it('should convert correctly', async () => {
			...
			const sUSDAmount = await ceth.maxLoan(toUnit('2.6'), hUSD);
	...
	describe('opening', async () => {
		describe('potential blocking conditions', async () => {
			['System', 'Issuance'].forEach(section => {
				describe(`when ${section} is suspended`, () => {
					...
					it('then calling openLoan() reverts', async () => {
						await assert.revert(
							ceth.open(onesUSD, hUSD, { value: twoETH, from: account1 }),
			...
			describe('when rates have gone stale', () => {
				...
				describe('when ETH gets a rate', () => {
					...
					it('then calling openLoan() succeeds', async () => {
						await ceth.open(onesUSD, hUSD, { value: twoETH, from: account1 });
					});
				});
			});
		});
		...
		describe('revert conditions', async () => {
			...
			it('should revert if they send 0 collateral', async () => {
				await assert.revert(
					ceth.open(onesUSD, hUSD, { value: oneETH, from: account1 }),
			...
			it('should revert if the requested loan exceeds borrowing power', async () => {
				await assert.revert(
					ceth.open(fiveHundredsUSD, hUSD, { value: twoETH, from: account1 }),
		...
		describe('should open an eth loan denominated in hUSD', async () => {
			beforeEach(async () => {
				tx = await ceth.open(fiveHundredsUSD, hUSD, {
					value: tenETH,
					from: account1,
				});
	...
	describe('withdraws', async () => {
		beforeEach(async () => {
			loan = await ceth.open(oneHundredsUSD, hUSD, {
				value: tenETH,
				from: account1,
			});
