...
contract('TribeoneBridgeToBase (unit tests)', accounts => {
	const [owner, user1, hakaBridgeToOptimism, smockedMessenger, randomAddress] = accounts;
	...
	describe('when all the deps are (s)mocked', () => {
		...
		beforeEach(async () => {
			...
			messenger.xDomainMessageSender.returns(() => hakaBridgeToOptimism);
		...
		describe('when the target is deployed', () => {
			...
			describe('withdraw', () => {
				describe('failure modes', () => {
					it('does not work when the user has less trasferable haka than the withdrawal amount', async () => {
				...
				describe('when invoked by a user', () => {
					...
					it('the message is relayed', async () => {
						...
						messenger.sendMessage.returnsAtCall(0, hakaBridgeToOptimism);
