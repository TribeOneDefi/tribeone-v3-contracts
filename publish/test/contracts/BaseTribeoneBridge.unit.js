		...
		describe('when the target is deployed and the proxy is set', () => {
			...
			describe('initiateSynthTransfer', () => {
				...
				describe('when enabled for cross chain transfer', () => {
					...
					beforeEach('run synth transfer calls', async () => {
						...
						txn = await instance.initiateSynthTransfer(hUSD, owner, toUnit('100'), { from: user1 });
					});
					...
					it('emits event', () => {
						assert.eventEqual(txn, 'InitiateSynthTransfer', [hUSD, owner, toUnit('100')]);
					});
				});
			});
			...
			describe('finalizeSynthTransfer', () => {
				...
				it('fails if xdomainmessagesender doesnt match counterpart', async () => {
					...
					await assert.revert(instance.finalizeSynthTransfer(hUSD, owner, '100'));
				});
				...
				describe('when successfully invoked', () => {
					...
					beforeEach(async () => {
						...
						txn = await instance.finalizeSynthTransfer(hUSD, user1, toUnit('125'), {
							from: smockedMessenger,
						});
					});
					...
					it('emits event', () => {
						assert.eventEqual(txn, 'FinalizeSynthTransfer', [hUSD, user1, toUnit('125')]);
					});
				});
			});
			...
			describe('synthTransferSent & synthTransferReceived', () => {
				beforeEach('set fake values', () => {
					...
					issuer.availableCurrencyKeys.returns([hUSD, sETH]);
