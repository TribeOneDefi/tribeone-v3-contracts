...
const purgeSynths = async ({
		...
		try {
			await confirmAction(
				cyan(
					`${yellow(
						'⚠ WARNING'
					)}: This action will purge the following synths from the TribeOne contract on ${network}:\n- ${synthsToPurge.join(
						'\n- '
					)}`
	...
	const { address: tribeoneAddress, source } = deployment.targets['TribeOne'];
	const { abi: tribeoneABI } = deployment.sources[source];
	const TribeOne = new ethers.Contract(tribeoneAddress, tribeoneABI, wallet);
	...
	for (const currencyKey of synthsToPurge) {
		...
		const currentSynthInHAKA = await TribeOne.synths(toBytes32(currencyKey));

		if (synthAddress !== currentSynthInHAKA) {
			console.error(
				red(
					`Synth address in TribeOne for ${currencyKey} is different from what's deployed in TribeOne to the local ${DEPLOYMENT_FILENAME} of ${network} \ndeployed: ${yellow(
						currentSynthInHAKA
