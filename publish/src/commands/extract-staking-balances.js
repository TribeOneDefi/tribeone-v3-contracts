...
async function extractStakingBalances({
	...
	const { abi: hakaABI } = getSource({ contract: 'Tribeone' });
	...
	const iSynthContract = getTarget({ contract: `Proxy${synth === 'hUSD' ? 'ERC20sUSD' : synth}` });
	...
	async function fetchStakedBalances() {
		const iSynth = new ethers.Contract(iSynthAddress, hakaABI, provider);
		const stakingContract = new ethers.Contract(stakingAddress, hakaABI, provider);
