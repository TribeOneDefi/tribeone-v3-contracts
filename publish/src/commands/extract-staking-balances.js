...
async function extractStakingBalances({
	...
	const iSynthContract = getTarget({ contract: `Proxy${synth === 'uUSD' ? 'ERC20uUSD' : synth}` });
	...
	// The exchange fee incurred when users are purged into uUSD
	const exchangeFee = await SystemSettings.exchangeFeeRate(toBytes32('uUSD'));

	console.log(gray(`Exchange fee of uUSD is`), yellow(ethers.utils.formatEther(exchangeFee)));
	...
	function computeOwedBalances(balances) {
		console.log(`\nComputing owed uUSD balances for accounts using parameters:`);
		...
		console.log(`${ethers.utils.formatEther(totalOwed, 'ether')} total uUSD owed.\n`);
		return result;
	}

	function saveOwedBalances(owedSUSDBalances) {
		let csvString = 'Address,Staked Balance,Owed uUSD,Readable Staked Balance,Readable Owed uUSD\n';
